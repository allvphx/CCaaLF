...
def main(args, encoder, state_size):
    ...
    return learn(command, cfg.get('log_directory'), state_size, args.pickup_policy)


def evaluate_encoder(encoder="./encoder/default_encoder_tpce.txt", state_size=0):
    # return 1, "test_load"
    ...
    parser.add_argument('--workload-type', type=str, default='tpce',
                        choices=['tpcc', 'tpce', 'ycsb'],
                        help='number of database workers')
    parser.add_argument('--nworkers', type=int, default=16, help='number of database workers')
    ...
    parser.add_argument('--scale-factor', type=int, default=1,
                        help='scale factor')
    parser.add_argument('--state-space', type=int, default=105,
                        help='state space for policy searching')
    parser.add_argument('--bench-opts', type=str, default='-w 0,0,0,0,0,0,50,0,0,50 -m 0 -s 1 -a 0',
                        help='benchmark info, e.g. workload mix ratio')
    parser.add_argument('--encoder', type=str, default='./encoder/default_encoder_tpcc.txt',
