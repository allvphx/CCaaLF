...
const size_t type_num = (end_type - trade_order_type);
...
#define TXN_MARKET_FEED_LIST(x) \
  x(81) \
  x(83) \
  x(90) \
  x(92)

#define TXN_MARKET_FEED_PIECE_RETRY(a) \
  case a: \
    { \
      goto piece_retry_##a; \
      break; \
...
tpce_worker::txn_market_feed()
{
  ...
  int mfi = 0;
  void *txn = db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_TPCE_MARKET_FEED, market_feed_type);

  // db->init_txn(txn, cgraph, market_feed_type);
  db->init_txn(txn, nullptr, market_feed_type, pg);
  ...
  try{
    ...
    // try
    {
      ...
      for (mfi = 0; mfi < marketfeedIndex; mfi++) {
        ...
        if (!(tbl_LAST_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 81 /*access_id*/))) {
          printf("symbol: %s\n", k.LT_S_SYMB.data());
        ...
        tbl_LAST_TRADE(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_lt_new), 82 /*access_id*/);
      }
    ...
    // goto market_feed_last_trade;
    // }
    expose_ret = db->expose_uncommitted(txn, 82 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_MARKET_FEED_LIST(TXN_MARKET_FEED_PIECE_RETRY)
    ...
    // try
    {
      ...
      for (mfi = 0; mfi < marketfeedIndex; mfi++) {
        ...
        tbl_TRADE_REQUEST_TR_S_SYMB(1)->scan(txn, Encode(obj_key0, k_lower), &Encode(obj_key1, k_upper),
                                             cb_stop_loss, s_arena.get(), 83 /*access_id*/);
        ...
        tbl_TRADE_REQUEST_TR_S_SYMB(1)->scan(txn, Encode(obj_key0, k_lower), &Encode(obj_key1, k_upper),
                                             cb_limit_sell, s_arena.get(), 84 /*access_id*/);
        ...
        tbl_TRADE_REQUEST_TR_S_SYMB(1)->scan(txn, Encode(obj_key0, k_lower), &Encode(obj_key1, k_upper),
                                             cb_limit_buy, s_arena.get(), 85 /*access_id*/);
        ...
        for (auto &it : request_list) {
          ...
          if (unlikely(!(tbl_TRADE_REQUEST(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 86 /*access_id*/)))) {
#ifdef EARLY_ABORT
            db->abort_txn(txn);
                        set_failed_records(db->get_failed_records(txn));
                        finished_txn_contention = db->get_txn_contention(txn);
                        is_user_initiate_abort = true;
                        return txn_result(false, market_feed_type);
#else
        ...
        for (auto &it : request_list) {
          if (!it.mark_deleted) {
            trade_request_t &tr = it;
            char *symbol = entry->symbol;
            ...
            k2.TR2_TR_T_ID_K = tr.trade_id;
            // delete TRADE_REQUEST
            tbl_TRADE_REQUEST(1/*TODO*/)->remove(txn, Encode(str(), k), 87 /*access_id*/);
            // delete secondary index TRADE_REQUEST_TR_B_ID
            tbl_TRADE_REQUEST_TR_B_ID(1/*TODO*/)->remove(txn, Encode(str(), k1), 88 /*access_id*/);
            // delete secondary index TRADE_REQUEST_TR_S_SYMB
            tbl_TRADE_REQUEST_TR_S_SYMB(1/*TODO*/)->remove(txn, Encode(str(), k2), 89 /*access_id*/);
          }
        }
      }
    ...
    // goto market_feed_trade_request;
    // }
    expose_ret = db->expose_uncommitted(txn, 89 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_MARKET_FEED_LIST(TXN_MARKET_FEED_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // Raw Piece: P3 -- w [TRADE]
    {
      for (auto &it : request_list) {
        ...
        TRADE::key k;
        k.T_ID = it.trade_id;
        ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 90 /*access_id*/));
        ...
        tbl_TRADE(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_t_new), 91 /*access_id*/);
      }
    }
    expose_ret = db->expose_uncommitted(txn, 91 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_MARKET_FEED_LIST(TXN_MARKET_FEED_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // try
    {
      ...
      for (auto &it : request_list) {
        ...
        v.TH_DTS = now_dts;
        tbl_TRADE_HISTORY(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 92 /*access_id*/);
        ret += Size(v);
      }
      // if (!db->mul_ops_end(txn)) {
    ...
    expose_ret = db->expose_uncommitted(txn, 92 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_MARKET_FEED_LIST(TXN_MARKET_FEED_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    if (likely(db->commit_txn(txn))){
      ...
      set_failed_records(db->get_failed_records(txn));
      finished_txn_contention = db->get_txn_contention(txn);
      return txn_result(true, market_feed_type);
    }
...
#define TXN_TRADE_ORDER_PIECE_RETRY(a) \
  case a: \
    { \
      goto piece_retry_##a; \
      break; \
...
tpce_worker::txn_trade_order()
{
  ...
  void *txn = db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_TPCE_TRADE_ORDER, trade_order_type);

  db->init_txn(txn, nullptr, trade_order_type, pg);
  ...
  try{
    ...
    char tax_status;
    {
      ...
      ALWAYS_ASSERT(tbl_CUSTOMER_ACCOUNT(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 0 /*access_id*/));
    ...
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_ORDER_LIST(TXN_TRADE_ORDER_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
      }
    }
    ...
    inline_str_fixed_tpce<TPCE::sm_cTAX_ID_len> tax_id;
    {
      ...
      CUSTOMER::key k;
      k.C_ID = cust_id;
      ALWAYS_ASSERT(tbl_CUSTOMER(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 1 /*access_id*/));
    ...
    piece_retry_2:
    {
      ...
      BROKER::key k;
      k.B_ID = broker_id;
      ALWAYS_ASSERT(tbl_BROKER(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 2 /*access_id*/));
    ...
    piece_retry_3:
    {
      ...
      if (strcmp(cust_l_name.data(), TxnReq.exec_l_name) != 0 ||
        ...
        if (unlikely(!tbl_ACCOUNT_PERMISSION(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 3 /*access_id*/))) {
          db->abort_txn(txn);
          set_failed_records(db->get_failed_records(txn));
          finished_txn_contention = db->get_txn_contention(txn);
          is_user_initiate_abort = true;
          return txn_result(false, trade_order_type);
        } else {
    ...
    // Raw Piece: P5 -- scan [SECURITY, COMPANY]
    {
      ...
      if (TxnReq.symbol[0] == '\0') {
        ...
        // scan [SECURITY_S_CO] r [SECURITY]
        {
          ...
          ALWAYS_ASSERT(tbl_SECURITY(1/*TODO*/)->get(txn, Encode(obj_key0, sk), obj_v, std::string::npos, 6 /*access_id*/));
        ...
        {
          const SECURITY::key k(symbol);
          ALWAYS_ASSERT(tbl_SECURITY(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 7 /*access_id*/));
        ...
        // r [COMPANY]
        {
          ...
          ALWAYS_ASSERT(tbl_COMPANY(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 8 /*access_id*/));
    ...
    // Raw Piece: P6 -- r [TRADE_TYPE]
    {
      ...
      ALWAYS_ASSERT(tbl_TRADE_TYPE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 9 /*access_id*/));
    ...
    // try
    {
      ...
      if (TxnReq.type_is_margin) {
        ...
        bool has_hs = tbl_HOLDING_SUMMARY(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 11 /*access_id*/);
      ...
      if (type_is_sell) { // sell order
        if (hs_qty > 0) {
          ...
          for (auto h_t_id : tid_set) {
            ...
            if (unlikely(!tbl_HOLDING(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 13 /*access_id*/))) {
#ifdef EARLY_ABORT
              db->abort_txn(txn);
                        set_failed_records(db->get_failed_records(txn));
                        finished_txn_contention = db->get_txn_contention(txn);
        ...
        if (hs_qty < 0) {
          ...
          for (auto h_t_id : tid_set) {
            const HOLDING::key k(h_t_id);
            if (unlikely(!tbl_HOLDING(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 15 /*access_id*/))) {
#ifdef EARLY_ABORT
              db->abort_txn(txn);
                        set_failed_records(db->get_failed_records(txn));
                        finished_txn_contention = db->get_txn_contention(txn);
    ...
    // try
    {
      ...
      if (TxnReq.type_is_margin) {
        ...
        for (auto &it : hs_list) {
          ...
          ALWAYS_ASSERT(tbl_LAST_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 16 /*access_id*/));
          LAST_TRADE::value v_lt_temp;
        ...
        const LAST_TRADE::key k(symbol);
        ALWAYS_ASSERT(tbl_LAST_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 17 /*access_id*/));
        LAST_TRADE::value v_lt_temp;
        const LAST_TRADE::value *v = Decode(obj_v, v_lt_temp);
        market_price = v->LT_PRICE;
      }
    ...
    // Raw Piece: P10 -- r [CUSTOMER_TAXRATE, TAX_RATE]
    {
      ...
      if (sell_value > buy_value && (tax_status == 1 || tax_status == 2)) {
        ...
        for (size_t si = 0; si < cb.size(); si++) {
          ...
          ALWAYS_ASSERT(tbl_TAX_RATE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 19 /*access_id*/));
    ...
    expose_ret = db->expose_uncommitted(txn, 20 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_ORDER_LIST(TXN_TRADE_ORDER_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // Raw Piece: P12 -- r [CHARGE]
    {
      ...
      ALWAYS_ASSERT(tbl_CHARGE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 21 /*access_id*/));
    ...
    // try
    {
      ...
      // insert TRADE
      tbl_TRADE(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 22 /*access_id*/);
      expose_ret = db->expose_uncommitted(txn, 22 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_ORDER_LIST(TXN_TRADE_ORDER_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      piece_retry_23:
      // we can not use insert_trade() directly, since we have to add partial rollback control
      // insert_trade(k, v, txn);
      TRADE_T_CA_ID::key k1;
      TRADE_T_CA_ID::value v1;
      k1.T1_T_CA_ID = v.T_CA_ID;
      k1.T1_T_DTS = v.T_DTS;
      k1.T1_T_ID_K = k.T_ID;
      v1.T1_T_ID_V = k.T_ID;
      // insert secondary index
      tbl_TRADE_T_CA_ID(1/*TODO*/)->insert(txn, Encode(str(), k1), Encode(str(), v1), 23 /*access_id*/);
      ...
      TRADE_T_S_SYMB::key k2;
      TRADE_T_S_SYMB::value v2;
      k2.T2_T_S_SYMB = v.T_S_SYMB;
      k2.T2_T_DTS = v.T_DTS;
      k2.T2_T_ID_K = k.T_ID;
      v2.T2_T_ID_V = k.T_ID;
      // insert secondary index
      tbl_TRADE_T_S_SYMB(1/*TODO*/)->insert(txn, Encode(str(), k2), Encode(str(), v2), 24 /*access_id*/);
      expose_ret = db->expose_uncommitted(txn, 24 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_ORDER_LIST(TXN_TRADE_ORDER_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
    ...
    // try
    {
      ...
      if (type_is_market) {
        ...
        // we can not use insert_trade_request() directly, since we have to add partial rollback control
        // insert_trade_request(k, v, txn);
        TRADE_REQUEST_TR_B_ID::key k1;
        TRADE_REQUEST_TR_B_ID::value v1;
        k1.TR1_TR_B_ID = v.TR_B_ID;
        k1.TR1_TR_S_SYMB = v.TR_S_SYMB;
        k1.TR1_TR_T_ID_K = k.TR_T_ID;
        v1.TR1_TR_T_ID_V = k.TR_T_ID;

        TRADE_REQUEST_TR_S_SYMB::key k2;
        TRADE_REQUEST_TR_S_SYMB::value v2;
        k2.TR2_TR_S_SYMB = v.TR_S_SYMB;
        k2.TR2_TR_TT_ID = v.TR_TT_ID;
        k2.TR2_TR_BID_PRICE = (int64_t)(v.TR_BID_PRICE*100);
        k2.TR2_TR_T_ID_K = k.TR_T_ID;
        v2.TR2_TR_T_ID_V = k.TR_T_ID;

        // insert TRADE_REQUEST
        tbl_TRADE_REQUEST(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 25 /*access_id*/);
        // insert secondary index
        tbl_TRADE_REQUEST_TR_B_ID(1/*TODO*/)->insert(txn, Encode(str(), k1), Encode(str(), v1), 26 /*access_id*/);
        // insert secondary index
        tbl_TRADE_REQUEST_TR_S_SYMB(1/*TODO*/)->insert(txn, Encode(str(), k2), Encode(str(), v2), 27 /*access_id*/);
    ...
    // goto trade_order_trade_request;
    // }
    expose_ret = db->expose_uncommitted(txn, 27 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_ORDER_LIST(TXN_TRADE_ORDER_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
...
// trade_order_trade_history:
    ...
    // try
    {
      ...
      tbl_TRADE_HISTORY(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 28 /*access_id*/);
      ret += Size(v);
      // if (!db->one_op_end(txn)) {
      // goto trade_order_trade_history;
      // }
    }
    expose_ret = db->expose_uncommitted(txn, 28 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_ORDER_LIST(TXN_TRADE_ORDER_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
...
// trade_order_customer_account:
    ...
    // try
    {
      ...
      if (TxnReq.type_is_margin) {
        CUSTOMER_ACCOUNT::key k;
        k.CA_ID = TxnReq.acct_id;
        ALWAYS_ASSERT(tbl_CUSTOMER_ACCOUNT(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 29 /*access_id*/));
        CUSTOMER_ACCOUNT::value v_ca_temp;
        const CUSTOMER_ACCOUNT::value *v = Decode(obj_v, v_ca_temp);
    ...
    expose_ret = db->expose_uncommitted(txn, 29 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_ORDER_LIST(TXN_TRADE_ORDER_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    if (likely(db->commit_txn(txn))){
      ...
      set_failed_records(db->get_failed_records(txn));
      finished_txn_contention = db->get_txn_contention(txn);
      return txn_result(true, trade_order_type);
    }
  } catch(transaction_abort_exception &ex) {
    db->abort_txn(txn);
  } catch (abstract_db::abstract_abort_exception &ex) {
...
#define TXN_TRADE_RESULT_LIST(x) \
  x(30) \
  x(31) \
  x(32) \
  x(33) \
...
#define TXN_TRADE_RESULT_PIECE_RETRY(a) \
  case a: \
    { \
      goto piece_retry_##a; \
      break; \
...
tpce_worker::txn_trade_result()
{
  ...
  void *txn = db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_TPCE_TRADE_RESULT, trade_result_type);
 ...
 *     } */
  ...
  db->init_txn(txn, nullptr, trade_result_type, pg);
  ...
  try{
    ...
    // Raw Piece: P1 -- r [TRADE]
    {
      ...
      ALWAYS_ASSERT(tbl_TRADE(1)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 30 /*access_id*/));
      TRADE::value v_t_temp;
    ...
    expose_ret = db->expose_uncommitted(txn, 30 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
      }
    }
    ...
    // Raw Piece: P2 -- r [TRADE_TYPE]
    {
      ...
      ALWAYS_ASSERT(tbl_TRADE_TYPE(1)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 31 /*access_id*/));
      TRADE_TYPE::value v_tt_temp;
      const TRADE_TYPE::value *v = Decode(obj_v, v_tt_temp);
      type_name = v->TT_NAME;
      type_is_sell = char2bool(v->TT_IS_SELL);
    ...
    expose_ret = db->expose_uncommitted(txn, 31 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // Raw Piece: P3 -- r [CUSTOMER_ACCOUNT]
    {
      ...
      ALWAYS_ASSERT(tbl_CUSTOMER_ACCOUNT(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 32 /*access_id*/));
      CUSTOMER_ACCOUNT::value v_ca_temp;
      const CUSTOMER_ACCOUNT::value *v = Decode(obj_v, v_ca_temp);
      broker_id = v->CA_B_ID;
      cust_id = v->CA_C_ID;
    ...
    expose_ret = db->expose_uncommitted(txn, 32 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // try
    {
      ...
      k.HS_CA_ID = acct_id;
      k.HS_S_SYMB = symbol;
      bool has_hs = tbl_HOLDING_SUMMARY(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 33 /*access_id*/);
      ...
      if (type_is_sell) {
        if (!has_hs) {
          ...
          vv.HS_QTY = -trade_qty;
          tbl_HOLDING_SUMMARY(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), vv), 34 /*access_id*/);
        } else {
          HOLDING_SUMMARY::value v_hs_new(*v);
          v_hs_new.HS_QTY = hs_qty - trade_qty;
          tbl_HOLDING_SUMMARY(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_hs_new), 35 /*access_id*/);
        }
      } else {
        if (!has_hs) {
          HOLDING_SUMMARY::value vv;
          vv.HS_QTY = trade_qty;
          tbl_HOLDING_SUMMARY(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), vv), 36 /*access_id*/);
        } else {
          HOLDING_SUMMARY::value v_hs_new(*v);
          v_hs_new.HS_QTY = hs_qty + trade_qty;
          tbl_HOLDING_SUMMARY(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_hs_new), 37 /*access_id*/);
        }
      }
      // if (!db->mul_ops_end(txn)) {
    ...
    // goto trade_result_holding_summary;
    // }
    expose_ret = db->expose_uncommitted(txn, 37 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
      ...
      if (type_is_sell) { // sell order
        if (hs_qty > 0) {
          ...
          if (unlikely(cb.size() == 0)) {
                    ...
                    finished_txn_contention = db->get_txn_contention(txn);
                    is_user_initiate_abort = true;
                    return txn_result(false, trade_result_type);
                }
#endif
          ...
          for (auto h_t_id : tid_set) {
            const HOLDING::key k(h_t_id);
            //need put afterwards
            if (unlikely(!(tbl_HOLDING(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 39 /*access_id*/)))) {
#ifdef EARLY_ABORT
              db->abort_txn(txn);
                        set_failed_records(db->get_failed_records(txn));
                        finished_txn_contention = db->get_txn_contention(txn);
            ...
            for (auto it = hold_tuple_list.rbegin();
              ...
              if (hold_qty >= needed_qty) {
                ...
                if (hold_qty == needed_qty) {
                  ...
                  HOLDING_H_CA_ID::key k;
                  k.H1_H_CA_ID = it->value->H_CA_ID;
                  k.H1_H_S_SYMB = it->value->H_S_SYMB;
                  k.H1_H_T_ID_K = it->key.H_T_ID;
                  tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(str(), k), 40 /*access_id*/);
                  tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(str(), it->key), 41 /*access_id*/);
                } else {
                  HOLDING::value v_h_new(*(it->value));
                  v_h_new.H_QTY -= needed_qty;
                  //add put here
                  tbl_HOLDING(1/*TODO*/)->put(txn, Encode(str(), it->key), Encode(str(), v_h_new), 42 /*access_id*/);
                }
                ...
                k.H1_H_S_SYMB = it->value->H_S_SYMB;
                k.H1_H_T_ID_K = it->key.H_T_ID;
                tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(str(), k), 43 /*access_id*/);
                tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(str(), it->key), 44 /*access_id*/);
            ...
            for (auto it = hold_tuple_list.begin();
              ...
              if (hold_qty >= needed_qty) {
                ...
                if (hold_qty == needed_qty) {
                  ...
                  k.H1_H_S_SYMB = it->value->H_S_SYMB;
                  k.H1_H_T_ID_K = it->key.H_T_ID;
                  tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(str(), k), 45 /*access_id*/);
                  tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(str(), it->key), 46 /*access_id*/);
                } else {
                  HOLDING::value v_h_new(*(it->value));
                  v_h_new.H_QTY -= needed_qty;
                  //add put here
                  tbl_HOLDING(1/*TODO*/)->put(txn, Encode(str(), it->key), Encode(str(), v_h_new), 47 /*access_id*/);
                }
                buy_value += needed_qty * hold_price;
                sell_value += needed_qty * TxnReq.trade_price;
                needed_qty = 0;
                ...
                k.H1_H_S_SYMB = it->value->H_S_SYMB;
                k.H1_H_T_ID_K = it->key.H_T_ID;
                tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(str(), k), 48 /*access_id*/);
                tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(str(), it->key), 49 /*access_id*/);
                buy_value += hold_qty * hold_price;
                sell_value += hold_qty * TxnReq.trade_price;
                needed_qty -= hold_qty;
              }
        ...
        if (needed_qty > 0) {
          ...
          // we can not use insert_holding() directly, since we have to add partial rollback control
          // insert_holding(k, v, txn);
          HOLDING_H_CA_ID::key kk;
          HOLDING_H_CA_ID::value vv;
          kk.H1_H_CA_ID = v.H_CA_ID;
          kk.H1_H_S_SYMB = v.H_S_SYMB;
          kk.H1_H_T_ID_K = k.H_T_ID;
          vv.H1_H_T_ID_V = k.H_T_ID;
          // insert secondary index
          tbl_HOLDING_H_CA_ID(1/*TODO*/)->insert(txn, Encode(str(), kk), Encode(str(), vv), 50/*access_id*/);
          // insert holding
          tbl_HOLDING(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 51/*access_id*/);
        }
      } else { // buy order
        if (hs_qty < 0) {
          // scan secondary index of HOLDING
          ...
          for (auto h_t_id : tid_set) {
            const HOLDING::key k(h_t_id);
            if (unlikely(!(tbl_HOLDING(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 53 /*access_id*/)))) {
#ifdef EARLY_ABORT
              db->abort_txn(txn);
                        set_failed_records(db->get_failed_records(txn));
                        finished_txn_contention = db->get_txn_contention(txn);
            ...
            for (auto it = hold_tuple_list.rbegin();
              ...
              if (hold_qty + needed_qty <= 0) {
                ...
                if (hold_qty + needed_qty == 0) {
                  ...
                  k.H1_H_S_SYMB = it->value->H_S_SYMB;
                  k.H1_H_T_ID_K = it->key.H_T_ID;
                  tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(str(), k), 54 /*access_id*/);
                  tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(str(), it->key), 55 /*access_id*/);
                } else {
                  HOLDING::value v_h_new(*(it->value));
                  v_h_new.H_QTY += needed_qty;
                  //add put here
                  tbl_HOLDING(1/*TODO*/)->put(txn, Encode(str(), it->key), Encode(str(), v_h_new), 56 /*access_id*/);
                }
                ...
                k.H1_H_S_SYMB = it->value->H_S_SYMB;
                k.H1_H_T_ID_K = it->key.H_T_ID;
                tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(str(), k), 57 /*access_id*/);
                tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(str(), it->key), 58 /*access_id*/);
                hold_qty = -hold_qty;
            ...
            for (auto it = hold_tuple_list.begin();
              ...
              if (hold_qty + needed_qty <= 0) {
                ...
                if (hold_qty + needed_qty == 0) {
                  ...
                  k.H1_H_S_SYMB = it->value->H_S_SYMB;
                  k.H1_H_T_ID_K = it->key.H_T_ID;
                  tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(str(), k), 59 /*access_id*/);
                  tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(str(), it->key), 60 /*access_id*/);
                } else {
                  HOLDING::value v_h_new(*(it->value));
                  v_h_new.H_QTY += needed_qty;
                  //add put here
                  tbl_HOLDING(1/*TODO*/)->put(txn, Encode(str(), it->key), Encode(str(), v_h_new), 61 /*access_id*/);
                }
                sell_value += needed_qty * hold_price;
                buy_value += needed_qty * TxnReq.trade_price;
                needed_qty = 0;
                ...
                k.H1_H_S_SYMB = it->value->H_S_SYMB;
                k.H1_H_T_ID_K = it->key.H_T_ID;
                tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(str(), k)), 62 /*access_id*/;
                tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(str(), it->key), 63 /*access_id*/);
                hold_qty = -hold_qty;
                sell_value += hold_qty * hold_price;
                buy_value += hold_qty * TxnReq.trade_price;
                needed_qty -= hold_qty;
        ...
        if (needed_qty > 0) {
          ...
          vv.H1_H_T_ID_V = k.H_T_ID;
          // insert secondary index
          tbl_HOLDING_H_CA_ID(1/*TODO*/)->insert(txn, Encode(str(), kk), Encode(str(), vv), 64/*access_id*/);
          // insert holding
          tbl_HOLDING(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 65/*access_id*/);
        }
      }
      // if (!db->mul_ops_end(txn)) {
    ...
    // goto trade_result_holding;
    // }
    expose_ret = db->expose_uncommitted(txn, 65 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // declared as 'std::list<std::pair<HOLDING_HISTORY::key, HOLDING_HISTORY::value> > hh_kv'
    {
      //ic3_txn->start_raw_piece();
      for (auto &it : hh_kv) {
        tbl_HOLDING_HISTORY(1/*TODO*/)->insert(txn, Encode(str(), it.first), Encode(str(), it.second), 66 /*access_id*/);
      }
      //ic3_txn->end_raw_piece();
    }
    expose_ret = db->expose_uncommitted(txn, 66 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    float tax_amount = 0.0;
    {
      ...
      if (sell_value > buy_value && (tax_status == 1 || tax_status == 2)) {
        ...
        for (size_t si = 0; si < cb.size(); si++) {
          ...
          const CUSTOMER_TAXRATE::key *kk = Decode(*cb.values[si].first, k_temp);
          const TAX_RATE::key k(kk->CX_TX_ID);
          ALWAYS_ASSERT(tbl_TAX_RATE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 68 /*access_id*/));
          TAX_RATE::value v_tr_temp;
          const TAX_RATE::value *v = Decode(obj_v, v_tr_temp);
          tax_rates += v->TX_RATE;
        }
    ...
    expose_ret = db->expose_uncommitted(txn, 68 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // Raw Piece: P8 -- r [SECURITY]
    {
      //ic3_txn->start_raw_piece();
      const SECURITY::key k(symbol);
      ALWAYS_ASSERT(tbl_SECURITY(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 69 /*access_id*/));
    ...
    expose_ret = db->expose_uncommitted(txn, 69 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // Raw Piece: P9 -- r [CUSTOMER]
    {
      ...
      ALWAYS_ASSERT(tbl_CUSTOMER(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 70 /*access_id*/));
      CUSTOMER::value v_c_temp;
      const CUSTOMER::value *v = Decode(obj_v, v_c_temp);
      cust_tier = v->C_TIER;
      //ic3_txn->end_raw_piece();
    }
    expose_ret = db->expose_uncommitted(txn, 70 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    expose_ret = db->expose_uncommitted(txn, 71 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // try
    {
      ...
      ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 72 /*access_id*/));
      TRADE::value v_t_temp;
      const TRADE::value *v = Decode(obj_v, v_t_temp);
      ...
      tbl_TRADE(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_t_new), 73 /*access_id*/);
      //ic3_txn->end_conflict_piece();
      // if (!db->one_op_end(txn)) {
      // goto trade_result_trade;
      // }
    }
    expose_ret = db->expose_uncommitted(txn, 73 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // try
    {
      ...
      TRADE_HISTORY::value v;
      v.TH_DTS = trade_dts;
      tbl_TRADE_HISTORY(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 74 /*access_id*/);
      // if (!db->one_op_end(txn)) {
      // goto trade_result_trade_history;
      // }
    }
    expose_ret = db->expose_uncommitted(txn, 74 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    { //TODO commutative
      ...
      BROKER::key k;
      k.B_ID = broker_id;
      ALWAYS_ASSERT(tbl_BROKER(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 75 /*access_id*/));
      BROKER::value v_b_temp;
      const BROKER::value *v = Decode(obj_v, v_b_temp);
      BROKER::value v_b_new(*v);
      v_b_new.B_COMM_TOTAL += comm_amount;
      v_b_new.B_NUM_TRADES++;
      tbl_BROKER(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_b_new), 76 /*access_id*/);
      // if (!db->one_op_end(txn)) {
      // goto trade_result_broker;
      // }
    }
    expose_ret = db->expose_uncommitted(txn, 76 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // try
    {
      ...
      v.SE_AMT = se_amount;
      tbl_SETTLEMENT(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 77 /*access_id*/);
      // if (!db->one_op_end(txn)) {
      // goto trade_result_settlement;
      // }
    }
    expose_ret = db->expose_uncommitted(txn, 77 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // try
    {
      ...
      CUSTOMER_ACCOUNT::key k;
      k.CA_ID = acct_id;
      ALWAYS_ASSERT(tbl_CUSTOMER_ACCOUNT(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 78 /*access_id*/));
      CUSTOMER_ACCOUNT::value v_ca_temp;
      const CUSTOMER_ACCOUNT::value *v = Decode(obj_v, v_ca_temp);
      CUSTOMER_ACCOUNT::value v_ca_new(*v);
      if (trade_is_cash)
        v_ca_new.CA_BAL += se_amount;
      tbl_CUSTOMER_ACCOUNT(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_ca_new), 79 /*access_id*/);
      // if (!db->one_op_end(txn)) {
      // goto trade_result_customer_account;
      // }
    }
    expose_ret = db->expose_uncommitted(txn, 79 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    // try
    {
      ...
      if (trade_is_cash) {
        ...
        tbl_CASH_TRANSACTION(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 80 /*access_id*/);
      }
      // if (!db->one_op_end(txn)) {
      // goto trade_result_cash_transaction;
      // }
    }
    expose_ret = db->expose_uncommitted(txn, 80 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
      switch (expose_ret.second) {
        TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
        default: ALWAYS_ASSERT(false);
    ...
    if (likely(db->commit_txn(txn))){
      set_failed_records(db->get_failed_records(txn));
      finished_txn_contention = db->get_txn_contention(txn);
      return txn_result(true, trade_result_type);
    }
  } catch(transaction_abort_exception &ex) {
    db->abort_txn(txn);
  } catch (abstract_db::abstract_abort_exception &ex) {
...
#define TXN_TRADE_UPDATE_LIST(x) \
  x(93) \
  x(96) \
  x(97) \
  x(98) \
...
#define TXN_TRADE_UPDATE_PIECE_RETRY(a) \
  case a: \
    { \
      goto piece_retry_##a; \
      break; \
...
tpce_worker::txn_trade_update()
{
  ...
  ssize_t ret = 0;
  void *txn = db->new_txn(txn_flags, arena, txn_buf(), abstract_db::HINT_TPCE_TRADE_UPDATE, trade_update_type);
  // db->init_txn(txn, cgraph, trade_update_type);
  db->init_txn(txn, nullptr, trade_update_type, pg);
  ...
  try {
    ...
    if (TxnReq.frame_to_execute == 1) {
      ...
      // try
      {
        ...
        for (int i = 0; i < TxnReq.max_trades; i++) {
          if (num_updated < TxnReq.max_updates) {
            ...
            ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k_t), obj_v, std::string::npos, 93 /*access_id*/));
            TRADE::value v_t_temp;
            const TRADE::value *v_t = Decode(obj_v, v_t_temp);
            ...
            num_updated++;
            tbl_TRADE(1/*TODO*/)->put(txn, Encode(str(), k_t), Encode(str(), v_t_new), 94 /*access_id*/);
          } else {
            const TRADE::key k_t(TxnReq.trade_id[i]);
            ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k_t), obj_v, std::string::npos, 95 /*access_id*/));
            TRADE::value v_t_temp;
            const TRADE::value *v_t = Decode(obj_v, v_t_temp);
            t_tt_id_list[i].assign(v_t->T_TT_ID.data(), v_t->T_TT_ID.size());
      ...
      expose_ret = db->expose_uncommitted(txn, 95 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
        }
      }
      // catch (piece_abort_exception &ex) {
      ...
      // try
      {
        ...
        for (int i = 0; i < TxnReq.max_trades; i++) {
          ...
          tbl_TRADE_HISTORY(1/*TODO*/)->scan(txn, Encode(obj_key0, k_th_0),
                                             &Encode(obj_key1, k_th_1), c, s_arena.get(), 96 /*access_id*/);
      ...
      expose_ret = db->expose_uncommitted(txn, 96 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      for (int i = 0; i < TxnReq.max_trades; i++) {
        ...
        ALWAYS_ASSERT(tbl_TRADE_TYPE(1/*TODO*/)->get(txn,
                                                     Encode(obj_key0, k_tt), obj_v, std::string::npos, 97 /*access_id*/));
        TRADE_TYPE::value v_tt_temp;
        const TRADE_TYPE::value *v_tt = Decode(obj_v, v_tt_temp);
        tt_is_mrkt = char2bool(v_tt->TT_IS_MRKT);
      }
      expose_ret = db->expose_uncommitted(txn, 97 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        for (int i = 0; i < TxnReq.max_trades; i++) {
          ...
          ALWAYS_ASSERT(tbl_SETTLEMENT(1/*TODO*/)->get(txn,
                                                       Encode(obj_key0, k_se), obj_v, std::string::npos, 98 /*access_id*/));
      ...
      expose_ret = db->expose_uncommitted(txn, 98 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
        ...
        for (int i = 0; i < TxnReq.max_trades; i++) {
          ...
          if (t_is_cash_list[i]) {
            ...
            ALWAYS_ASSERT(tbl_CASH_TRANSACTION(1/*TODO*/)->get(txn,
                                                               Encode(obj_key0, k_ct), obj_v, std::string::npos, 99 /*access_id*/));
      ...
      expose_ret = db->expose_uncommitted(txn, 99 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        tbl_TRADE_T_CA_ID(1/*TODO*/)->scan(txn, Encode(obj_key0, k_tca_0),
                                           &Encode(obj_key1, k_tca_1), c, s_arena.get(),100 /*access_id*/);

        expose_ret = db->expose_uncommitted(txn, 100 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
          switch (expose_ret.second) {
            TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
            default: ALWAYS_ASSERT(false);
        ...
        for (size_t i = 0; i < c.size(); i++){
          //UNUSED float t_bid_price, t_trade_price;
          {
            ...
            ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k_t), obj_v, std::string::npos, 101 /*access_id*/));
            TRADE::value v_t_temp;
            const TRADE::value *v_t = Decode(obj_v, v_t_temp);
      ...
      // goto trade_update_trade_f2;
      // }
      expose_ret = db->expose_uncommitted(txn, 101 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        for (uint32_t i = 0; i < trade_id.size(); i++) {
          ...
          NDB_MEMSET((char*)k_th_1.TH_ST_ID.data(), max_char, TPCE::sm_cST_ID_len);
          tbl_TRADE_HISTORY(1/*TODO*/)->scan(txn, Encode(obj_key0, k_th_0),
                                             &Encode(obj_key1, k_th_1), c, s_arena.get(), 102 /*access_id*/);
          for (size_t j = 0; j < c.size(); j++) {
            TRADE_HISTORY::key k_th_temp;
            const TRADE_HISTORY::key *k_th_t = Decode(*c.values[j].first, k_th_temp);
            //th_st_id[th_len].assign(k_th_t->TH_ST_ID.data(), k_th_t->TH_ST_ID.size());
      ...
      // goto trade_update_trade_history_f2;
      // }
      expose_ret = db->expose_uncommitted(txn, 102 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        for (uint32_t i = 0; i < trade_id.size(); i++) {
          ...
          if (num_updated < TxnReq.max_updates) {
            ALWAYS_ASSERT(tbl_SETTLEMENT(1/*TODO*/)->get(txn,
                                                         Encode(obj_key0, k_se), obj_v, std::string::npos, 103 /*access_id*/));
            SETTLEMENT::value v_se_temp;
            const SETTLEMENT::value *v_se = Decode(obj_v, v_se_temp);
            ...
            tbl_SETTLEMENT(1/*TODO*/)->put(txn, Encode(str(), k_se),
                                           Encode(obj_v, v_se_new), 104 /*access_id*/);
          }else{
            ALWAYS_ASSERT(tbl_SETTLEMENT(1/*TODO*/)->get(txn,
                                                         Encode(obj_key0, k_se), obj_v, std::string::npos, 105 /*access_id*/));
            SETTLEMENT::value v_se_temp;
            const SETTLEMENT::value *v_se = Decode(obj_v, v_se_temp);
            ALWAYS_ASSERT(v_se);
            //se_amt = v_se->SE_AMT;
      ...
      expose_ret = db->expose_uncommitted(txn, 105 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        for (uint32_t i = 0; i < trade_id.size(); i++) {
          ...
          if (t_is_cash_list[i]) {
            ...
            ALWAYS_ASSERT(tbl_CASH_TRANSACTION(1/*TODO*/)->get(txn,
                                                               Encode(obj_key0, k_ct), obj_v, std::string::npos, 106 /*access_id*/));
            CASH_TRANSACTION::value v_ct_temp;
            const CASH_TRANSACTION::value *v_ct = Decode(obj_v, v_ct_temp);
            ALWAYS_ASSERT(v_ct);
            //ct_name.assign(v_ct->CT_NAME.data(), v_ct->CT_NAME.size());
      ...
      expose_ret = db->expose_uncommitted(txn, 106 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        tbl_TRADE_T_S_SYMB(1/*TODO*/)->scan(txn, Encode(obj_key0, k_ts_0),
                                            &Encode(obj_key1, k_ts_1), c, s_arena.get(), 107 /*access_id*/);

        expose_ret = db->expose_uncommitted(txn, 107 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
          switch (expose_ret.second) {
            TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
            default: ALWAYS_ASSERT(false);
        ...
        for(size_t i = 0; i < c.size(); i++){
...

          {
            ...
            ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn,
                                                    Encode(obj_key0, k_t), obj_v, std::string::npos, 108 /*access_id*/));
            TRADE::value v_t_temp;
            const TRADE::value *v_t = Decode(obj_v, v_t_temp);
            //t_ca_id = v_t->T_CA_ID;
        ...
        // goto trade_update_trade_f3;
        // }
        expose_ret = db->expose_uncommitted(txn, 108 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
          switch (expose_ret.second) {
            TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
            default: ALWAYS_ASSERT(false);
      ...
      // Raw Piece: P2 [TRADE_TYPE]
      {
        for (uint32_t i = 0; i < trade_id.size(); i++) {
          ...
          const TRADE_TYPE::key k_tt(t_tt_id_list[i].c_str());
          ALWAYS_ASSERT(tbl_TRADE_TYPE(1/*TODO*/)->get(txn,
                                                       Encode(obj_key0, k_tt), obj_v, std::string::npos, 109 /*access_id*/));
          TRADE_TYPE::value v_tt_temp;
          const TRADE_TYPE::value *v_tt = Decode(obj_v, v_tt_temp);
          ALWAYS_ASSERT(v_tt);
      ...
      expose_ret = db->expose_uncommitted(txn, 109 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // Raw Piece: P3 [SECURITY]
      {
        ...
        ALWAYS_ASSERT(tbl_SECURITY(1/*TODO*/)->get(txn,
                                                   Encode(obj_key0, k_s), obj_v, std::string::npos, 110 /*access_id*/));
        SECURITY::value v_s_temp;
        const SECURITY::value *v_s = Decode(obj_v, v_s_temp);
        ALWAYS_ASSERT(v_s);
        //s_name.assign(v_s->S_NAME.data(), v_s->S_NAME.size());
      }
      expose_ret = db->expose_uncommitted(txn, 110 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        for (uint32_t i = 0; i < trade_id.size(); i++) {
          ...
          tbl_TRADE_HISTORY(1/*TODO*/)->scan(txn, Encode(obj_key0, k_th_0),
                                             &Encode(obj_key1, k_th_1), c, s_arena.get(), 111 /*access_id*/);
          for (size_t j = 0; j < c.size(); j++) {
            TRADE_HISTORY::key k_th_temp;
            const TRADE_HISTORY::key *k_th_t = Decode(*c.values[j].first, k_th_temp);
            //th_st_id[th_len].assign(k_th_t->TH_ST_ID.data(), k_th_t->TH_ST_ID.size());
      ...
      // goto trade_update_trade_history_f3;
      // }
      expose_ret = db->expose_uncommitted(txn, 111 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        for (uint32_t i = 0; i < trade_id.size(); i++) {
          ...
          const SETTLEMENT::key k_se(trade_id[i]);
          ALWAYS_ASSERT(tbl_SETTLEMENT(1/*TODO*/)->get(txn,
                                                       Encode(obj_key0, k_se), obj_v, std::string::npos, 112 /*access_id*/));
          SETTLEMENT::value v_se_temp;
          const SETTLEMENT::value *v_se = Decode(obj_v, v_se_temp);
          ALWAYS_ASSERT(v_se);
          //se_cash_type.assign(v_se->SE_CASH_TYPE.data(), v_se->SE_CASH_TYPE.size());
      ...
      // goto trade_update_settlement_f3;
      // }
      expose_ret = db->expose_uncommitted(txn, 112 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
      ...
      // try
      {
        ...
        for (uint32_t i = 0; i < trade_id.size(); i++) {
          ...
          if (t_is_cash_list[i]) {
            ...
            if(num_updated < TxnReq.max_updates){
              ALWAYS_ASSERT(tbl_CASH_TRANSACTION(1/*TODO*/)->get(txn,
                                                                 Encode(obj_key0, k_ct), obj_v, std::string::npos, 113 /*access_id*/));
              CASH_TRANSACTION::value v_ct_temp;
              const CASH_TRANSACTION::value *v_ct = Decode(obj_v, v_ct_temp);
              ...
              tbl_CASH_TRANSACTION(1/*TODO*/)->put(txn,
                                                   Encode(str(), k_ct), Encode(obj_v, v_ct_new), 114 /*access_id*/);
            } else {
              ALWAYS_ASSERT(tbl_CASH_TRANSACTION(1/*TODO*/)->get(txn,
                                                                 Encode(obj_key0, k_ct), obj_v, std::string::npos, 115 /*access_id*/));
              CASH_TRANSACTION::value v_ct_temp;
              const CASH_TRANSACTION::value *v_ct = Decode(obj_v, v_ct_temp);
              ALWAYS_ASSERT(v_ct);
              //ct_name.assign(v_ct->CT_NAME.data(), v_ct->CT_NAME.size());
      ...
      // goto trade_update_cash_transaction_f3;
      // }
      expose_ret = db->expose_uncommitted(txn, 115 + ACCESSES /*access_id*/);
      if (!expose_ret.first) {
        switch (expose_ret.second) {
          TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
          default: ALWAYS_ASSERT(false);
    ...
    if (likely(db->commit_txn(txn))){
      set_failed_records(db->get_failed_records(txn));
      finished_txn_contention = db->get_txn_contention(txn);
      return txn_result(true, trade_update_type);
    }
  } catch(transaction_abort_exception &ex) {
    db->abort_txn(txn);
  } catch (abstract_db::abstract_abort_exception &ex) {
  ...
  set_failed_records(db->get_failed_records(txn));
  finished_txn_contention = db->get_txn_contention(txn);
  return txn_result(false, trade_update_type);
}
