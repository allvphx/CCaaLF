...
tpcc_worker::txn_delivery()
{
  ...
  try {
    ...
    // access 1: remove the read new order records.
    access_id ++;
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
    ...
    // access 2: read modify write the order table.
    access_id ++;
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      if(last_no_o_id[d - 1] == -1)
      {
    ...
    // access 3: update order line tables for delivered orders.
    access_id ++;
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      if(last_no_o_id[d - 1] == -1)
      {
    ...
    // access 4: update customer tables for balance.
    access_id ++;
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      if(last_no_o_id[d - 1] == -1)
      {
...
tpcc_worker::txn_payment()
{
  ...
  try {
    ...
    // access 1: read modify write warehouse table, update YTD
    access_id ++;
    const warehouse::key k_w(warehouse_id);
    ...
    // access 2: read modify write district table, update total payment amount.
    access_id ++;
    const district::key k_d(warehouse_id, districtID);
    ALWAYS_ASSERT(tbl_district(warehouse_id)->get(txn, Encode(obj_key0, k_d), obj_v, D_SIZE, access_id));
    district::value v_d_temp;
    ...
    // access 3: read modify write user (by name or by id), update user balance.
    access_id ++;
    customer::key k_c;
    customer::value v_c;
