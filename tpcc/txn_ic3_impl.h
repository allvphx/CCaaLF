...
// forward decl
template <typename Traits> class transaction_ic3;
...
class txn_logger {
  friend class transaction_ic3_static;
  template <typename T>
    friend class transaction_ic3;
  // XXX: should only allow txn_epoch_sync<transaction_ic3> as friend
...
txn_logger::pbuffer::can_hold_tid(uint64_t tid) const
{
  return !header()->nentries_ ||
         (transaction_ic3_static::EpochId(header()->last_tid_) ==
          transaction_ic3_static::EpochId(tid));
}

// protocol 2 - no global consistent TIDs
template <typename Traits>
class transaction_ic3 : public transaction<transaction_ic3, Traits>,
                           private transaction_ic3_static {

  friend class transaction<transaction_ic3, Traits>;
  typedef transaction<transaction_ic3, Traits> super_type;
...
public:
  ...
  transaction_ic3(uint64_t flags,
                     typename Traits::StringAllocator &sa, uint8_t txn_type = 0)
    : transaction<transaction_ic3, Traits>(flags, sa, txn_type)
  {
  ...
  can_overwrite_record_tid(tid_t prev, tid_t cur) const
  {
    ...
    return prev == dbtuple::MAX_TID ||
           (to_read_only_tick(EpochId(prev)) ==
...
public:
  ...
  {
    transaction<transaction_ic3, Traits>::dump_debug_info();
    if (this->is_snapshot())
  ...
  gen_commit_tid()
  {
    ...
    // if(!this->commutative_set.empty())
    // {
    //   auto it  = this->commutative_set.begin();
    //   for (; it != this->commutative_set.end(); ++it) {
...

    {
      ...
      for (; it != it_end; ++it) {
        if (it->is_occ() || it->get_dep_txn() == nullptr) {
          ...
          tid_t version = it->get_tuple()->version;
          if (version > ret)
            ret = version;
        }
      }
    ...
    if(!this->write_set.empty())
    {
      typename write_set_map::iterator it     = this->write_set.begin();
      typename write_set_map::iterator it_end = this->write_set.end();
      for (; it != it_end; ++it) {
        // if (it->is_insert())
        //   continue;
        const tid_t t = it->get_tuple()->version;
  ...
  on_dbtuple_spill(dbtuple *tuple_ahead, dbtuple *tuple)
  {
    ...
    INVARIANT(tuple->version == dbtuple::MAX_TID ||
              tuple_ahead->version >= tuple->version);
    ...
    INVARIANT(tuple->version == dbtuple::MAX_TID ||
              to_read_only_tick(EpochId(tuple->version)) <= ro_tick);
...
template <>
struct base_txn_btree_handler<transaction_ic3> {
  static inline void
  on_construct()
  {
...
template <>
struct txn_epoch_sync<transaction_ic3> : public transaction_ic3_static {
  static void
  sync()
  {
    wait_an_epoch();
