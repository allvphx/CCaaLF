...
protected:
  ...
  struct read_record_t {
    ...
    clean_tid()
    {
      // XXX(xxx):return the clean tid of the tuple for clean read validation
...
public:
  ...
  inline transaction(uint64_t flags, string_allocator_type &sa, uint8_t txn_type);
  inline ~transaction();

  inline void
  init(conflict_graph* cg, uint8_t t, Policy *p = nullptr)
  {
    assert(txn_type == t);
    c_graph = cg;
    feature = new xact(tid, txn_type);
    pg = p;
    mix_op.set_pg(p);
    finish_init();
  ...
  bool can_order_after(uint64_t t, double t_rank, bool wait_self) {
    if (t != tid) return true;
    auto rk = txn_current_rank;
    if (rk < 0.1 || rk < t_rank) return true;
    if (!wait_self && fabs(rk - t_rank) < 1e-3) return true;
    return false;
  }
...
public:
  ...
  inline void update_txn_step(uint32_t acc_id) {
    if (txn_type == 0 || acc_id == MAX_ACC_ID)
      return;
  ...
  ALWAYS_INLINE void before_access_operation(PolicyAction *pa = nullptr) {
    if (pa == nullptr) return;
    txn_current_rank = pa->rank;
    do_wait(pa->access,
            pa->rank,
            pa->timeout,
            pa->safeguard);
  ...
  ALWAYS_INLINE void before_commit_piece_operation(PolicyAction *pa, bool is_final) {
    if(pa == nullptr) return;
    if (unlikely(is_final)) {
      do_wait(before_commit_policy.access,
              before_commit_policy.rank,
              before_commit_policy.timeout,
              before_commit_policy.safeguard);
    } else {
      do_wait(pa->expose_access,
              pa->expose_rank,
              pa->expose_timeout,
              pa->expose_safeguard);
    }
  ...
  ALWAYS_INLINE bool inference_commit_piece(PolicyAction *pa = nullptr) {
    if (pa == nullptr) return false;
    return pa->expose;
  }

  // return true to do the piece commit
  ALWAYS_INLINE bool inference_scan_access(PolicyAction *pa = nullptr) {
    if (pa == nullptr) return false;
    return pa->access == no_detect;
  }
  ...
  ALWAYS_INLINE PolicyAction* refresh_policy(uint32_t acc_id, OpType type) {
    if (acc_id == MAX_ACC_ID || type == OpNone || txn_type == 0) {
      return nullptr;
    }
    if (acc_id >= ACCESSES) {
      assert(type == OpCommit);
      acc_id -= ACCESSES;
    }
    feature->tx_n_op = acc_id - txn_first_acc_id;
    feature->cur_acc_id  = acc_id;
    feature->tx_cur_op = type;
#if COUNT_TX_BLOCK
    feature->tx_n_dep_by = txn_current_blocking;
#endif
    return feature->get_cur_policy(pg);
  }
  ...
  void do_wait(AccessPolicy access, double rank, uint32_t timeout, uint32_t safeguard[TXN_TYPE]);
...
public:
  ...
  const tid_t tid;
  uint64_t cur_step;
  volatile double txn_current_rank = lowest_priority;
#if COUNT_TX_BLOCK
  uint32_t txn_current_blocking = 0;
#endif
