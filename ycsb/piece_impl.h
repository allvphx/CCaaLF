...
//=================================MIX Operation================================
 ...
 template <typename Transaction>
 void mix_op<Transaction>::set_pg(Policy *pgg) {
   pg = pgg;
 }
  ...
  uint32_t acc_id)
{
  ...
  txn->update_txn_step(acc_id);
  auto pa = txn->refresh_policy(acc_id, OpRead);
  if (unlikely(acc_id == MAX_ACC_ID || pg == nullptr || pa == nullptr)) {
    occ = true;
    lock_mode = true;
  } else {
    occ = pa->access == no_detect;
  }
  // before access 
  txn->before_access_operation(pa);
  ...
  uint32_t acc_id)
{
  ...
  bool occ, lock_mode = true, is_insert = false;
  txn->update_txn_step(acc_id);
  auto pa = txn->refresh_policy(acc_id, OpUpdate);
  if (unlikely(acc_id == MAX_ACC_ID || pg == nullptr || pa == nullptr)) {
     occ = true;
     lock_mode = true;
  } else {
...
retry:
  if (expect_new) {
    ...
    if (px) {
      ...
      ALWAYS_ASSERT(node);
      txn->insert_write_set(px, k, v, writer, &btr, true, nullptr, txn, occ, record_contention, txn->get_tid(), dbtuple::MIN_TID, node);
      is_insert = true;
  ...
  if (!px) {
    ...
    txn->before_access_operation(pa);
    txn->insert_write_set(px, k, v, writer, &btr, false, nullptr, txn, occ, record_contention, txn->get_tid());
  }
  return true;
}
...
//  }
  ...
  // XXX(xxx): Hack - everything simply use MIX
