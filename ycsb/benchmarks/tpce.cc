...
piece_retry_78:
        // try 
        {
            ...
            for (mfi = 0; mfi < marketfeedIndex; mfi++) {
                ...
                if (!(tbl_LAST_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 78 /*access_id*/))) {
                    printf("symbol: %s\n", k.LT_S_SYMB.data());
                ...
                tbl_LAST_TRADE(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_lt_new), 78 /*access_id*/);
            }
        ...
        // catch (piece_abort_exception &ex) {
            ...
            // goto market_feed_last_trade;
        // }
        expose_ret = db->expose_uncommitted(txn, 78 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
...
piece_retry_79:
        ...
        // try 
        {
            ...
            for (mfi = 0; mfi < marketfeedIndex; mfi++) {
                ...
                tbl_TRADE_REQUEST_TR_S_SYMB(1)->scan(txn, Encode(obj_key0, k_lower), &Encode(obj_key1, k_upper),
                        cb_stop_loss, s_arena.get(), 79 /*access_id*/);
                ...
                tbl_TRADE_REQUEST_TR_S_SYMB(1)->scan(txn, Encode(obj_key0, k_lower), &Encode(obj_key1, k_upper),
                        cb_limit_sell, s_arena.get(), 80 /*access_id*/);
                ...
                tbl_TRADE_REQUEST_TR_S_SYMB(1)->scan(txn, Encode(obj_key0, k_lower), &Encode(obj_key1, k_upper),
                        cb_limit_buy, s_arena.get(), 81 /*access_id*/);
                ...
                for (auto &it : request_list) {
                    ...
                    if (unlikely(!(tbl_TRADE_REQUEST(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 82 /*access_id*/)))) {
#ifdef EARLY_ABORT
                        db->abort_txn(txn);
                ...
                for (auto &it : request_list) {
                    if (!it.mark_deleted) {
                        ...
                        k2.TR2_TR_T_ID_K = tr.trade_id;
                        // delete TRADE_REQUEST
                        tbl_TRADE_REQUEST(1/*TODO*/)->remove(txn, Encode(str(), k), 83 /*access_id*/);
                        // delete secondary index TRADE_REQUEST_TR_B_ID
                        tbl_TRADE_REQUEST_TR_B_ID(1/*TODO*/)->remove(txn, Encode(str(), k1), 84 /*access_id*/);
                        // delete secondary index TRADE_REQUEST_TR_S_SYMB
                        tbl_TRADE_REQUEST_TR_S_SYMB(1/*TODO*/)->remove(txn, Encode(str(), k2), 85 /*access_id*/);
                    }
                }
            }
        ...
        // catch (piece_abort_exception &ex) {
            ...
            // goto market_feed_trade_request;
        // }
        expose_ret = db->expose_uncommitted(txn, 85 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
            switch (expose_ret.second) {
                TXN_MARKET_FEED_LIST(TXN_MARKET_FEED_PIECE_RETRY)
                default: ALWAYS_ASSERT(false);
...
piece_retry_86:
		// Raw Piece: P3 -- w [TRADE]
		{
			for (auto &it : request_list) {
				...
				TRADE::key k;
				k.T_ID = it.trade_id;
	      		ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 86 /*access_id*/));
			    ...
			    tbl_TRADE(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_t_new), 86 /*access_id*/);
			}
		}
        expose_ret = db->expose_uncommitted(txn, 86 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
            switch (expose_ret.second) {
                TXN_MARKET_FEED_LIST(TXN_MARKET_FEED_PIECE_RETRY)
                default: ALWAYS_ASSERT(false);
...
piece_retry_87:
		// try 
        {
			...
			for (auto &it : request_list) {
				...
				v.TH_DTS = now_dts;
				tbl_TRADE_HISTORY(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 87 /*access_id*/);
				ret += Size(v);
			}
            // if (!db->mul_ops_end(txn)) {
        ...
        expose_ret = db->expose_uncommitted(txn, 87 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
            switch (expose_ret.second) {
                TXN_MARKET_FEED_LIST(TXN_MARKET_FEED_PIECE_RETRY)
                default: ALWAYS_ASSERT(false);
...
piece_retry_72:
    // try 
    {
		...
		tbl_TRADE(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_t_new), 72 /*access_id*/);
        //ic3_txn->end_conflict_piece();
        // if (!db->one_op_end(txn)) {
            // goto trade_result_trade;
        // }
    }
    expose_ret = db->expose_uncommitted(txn, 72 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
        switch (expose_ret.second) {
            TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
            default: ALWAYS_ASSERT(false);
...
piece_retry_73:
    // try 
    {
        ...
        TRADE_HISTORY::value v;
        v.TH_DTS = trade_dts;
		tbl_TRADE_HISTORY(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 73 /*access_id*/);
        // if (!db->one_op_end(txn)) {
...
piece_retry_74:
    ...
    { //TODO commutative
        ...
        BROKER::key k;
        k.B_ID = broker_id;
		ALWAYS_ASSERT(tbl_BROKER(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 74 /*access_id*/));
        BROKER::value v_b_temp;
        const BROKER::value *v = Decode(obj_v, v_b_temp);
		BROKER::value v_b_new(*v);
        v_b_new.B_COMM_TOTAL += comm_amount;
        v_b_new.B_NUM_TRADES++;
		tbl_BROKER(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_b_new), 74 /*access_id*/);
        // if (!db->one_op_end(txn)) {
...
piece_retry_75:
    // try 
    {
        ...
        v.SE_AMT = se_amount;
		tbl_SETTLEMENT(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 75 /*access_id*/);
        // if (!db->one_op_end(txn)) {
            // goto trade_result_settlement;
        // }
    } 
    expose_ret = db->expose_uncommitted(txn, 75 + ACCESSES /*access_id*/);
    if (!expose_ret.first) {
        switch (expose_ret.second) {
            TXN_TRADE_RESULT_LIST(TXN_TRADE_RESULT_PIECE_RETRY)
            default: ALWAYS_ASSERT(false);
...
piece_retry_76:
    // try 
    {
        ...
        CUSTOMER_ACCOUNT::key k;
        k.CA_ID = acct_id;
		ALWAYS_ASSERT(tbl_CUSTOMER_ACCOUNT(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v, std::string::npos, 76 /*access_id*/));
		CUSTOMER_ACCOUNT::value v_ca_temp;
        const CUSTOMER_ACCOUNT::value *v = Decode(obj_v, v_ca_temp);
		CUSTOMER_ACCOUNT::value v_ca_new(*v);
        if (trade_is_cash)
            v_ca_new.CA_BAL += se_amount;
		tbl_CUSTOMER_ACCOUNT(1/*TODO*/)->put(txn, Encode(str(), k), Encode(str(), v_ca_new), 76 /*access_id*/);
        // if (!db->one_op_end(txn)) {
...
piece_retry_77:
    // try 
    {
        ...
        if (trade_is_cash) {
			...
			tbl_CASH_TRANSACTION(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(str(), v), 77 /*access_id*/);
        }
        // if (!db->one_op_end(txn)) {
...
piece_retry_88:
        ...
        // try 
        {
            ...
            for (int i = 0; i < TxnReq.max_trades; i++) {
                if (num_updated < TxnReq.max_updates) {
                    ...
                    ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k_t), obj_v, std::string::npos, 88 /*access_id*/));
                    TRADE::value v_t_temp;
                    const TRADE::value *v_t = Decode(obj_v, v_t_temp);
                    ...
                    num_updated++;
                    tbl_TRADE(1/*TODO*/)->put(txn, Encode(str(), k_t), Encode(str(), v_t_new), 88 /*access_id*/);
                } else {
                    const TRADE::key k_t(TxnReq.trade_id[i]);
                    ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k_t), obj_v, std::string::npos, 88 /*access_id*/));
                    TRADE::value v_t_temp;
        ...
        expose_ret = db->expose_uncommitted(txn, 88 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
            switch (expose_ret.second) {
        ...
        // try 
        {
            ...
            for (int i = 0; i < TxnReq.max_trades; i++) {
                ...
                tbl_TRADE_HISTORY(1/*TODO*/)->scan(txn, Encode(obj_key0, k_th_0),
                        &Encode(obj_key1, k_th_1), c, s_arena.get(), 89 /*access_id*/);
...
piece_retry_90:
        for (int i = 0; i < TxnReq.max_trades; i++) {
            ...
            ALWAYS_ASSERT(tbl_TRADE_TYPE(1/*TODO*/)->get(txn,
                        Encode(obj_key0, k_tt), obj_v, std::string::npos, 90 /*access_id*/));
            TRADE_TYPE::value v_tt_temp;
            const TRADE_TYPE::value *v_tt = Decode(obj_v, v_tt_temp);
            tt_is_mrkt = char2bool(v_tt->TT_IS_MRKT);
        }
        expose_ret = db->expose_uncommitted(txn, 90 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
            switch (expose_ret.second) {
                TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
                default: ALWAYS_ASSERT(false);
        ...
        // try 
        {
			...
			for (int i = 0; i < TxnReq.max_trades; i++) {
				...
				ALWAYS_ASSERT(tbl_SETTLEMENT(1/*TODO*/)->get(txn,
                            Encode(obj_key0, k_se), obj_v, std::string::npos, 91 /*access_id*/));
			...
			for (int i = 0; i < TxnReq.max_trades; i++) {
				...
				if (t_is_cash_list[i]) {
					...
					ALWAYS_ASSERT(tbl_CASH_TRANSACTION(1/*TODO*/)->get(txn,
                                Encode(obj_key0, k_ct), obj_v, std::string::npos, 92 /*access_id*/));
...
piece_retry_93:
        ...
        // try 
        {
            ...
            tbl_TRADE_T_CA_ID(1/*TODO*/)->scan(txn, Encode(obj_key0, k_tca_0),
                    &Encode(obj_key1, k_tca_1), c, s_arena.get(), 93 /*access_id*/);
            ...
            for (size_t i = 0; i < c.size(); i++){
                //UNUSED float t_bid_price, t_trade_price;
                {
                    ...
                    ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k_t), obj_v, std::string::npos, 94 /*access_id*/));
                    TRADE::value v_t_temp;
                    const TRADE::value *v_t = Decode(obj_v, v_t_temp);
        ...
        // catch (piece_abort_exception &ex) {
            ...
            // goto trade_update_trade_f2;
        // }
        expose_ret = db->expose_uncommitted(txn, 94 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
            switch (expose_ret.second) {
                TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
                default: ALWAYS_ASSERT(false);
...
piece_retry_95:
        // try 
        {
            ...
            for (uint32_t i = 0; i < trade_id.size(); i++) {
                ...
                NDB_MEMSET((char*)k_th_1.TH_ST_ID.data(), max_char, TPCE::sm_cST_ID_len);
                tbl_TRADE_HISTORY(1/*TODO*/)->scan(txn, Encode(obj_key0, k_th_0),
                        &Encode(obj_key1, k_th_1), c, s_arena.get(), 95 /*access_id*/);
                for (size_t j = 0; j < c.size(); j++) {
                    TRADE_HISTORY::key k_th_temp;
                    const TRADE_HISTORY::key *k_th_t = Decode(*c.values[j].first, k_th_temp);
                    //th_st_id[th_len].assign(k_th_t->TH_ST_ID.data(), k_th_t->TH_ST_ID.size());
...
piece_retry_96:
        // try 
        {
            ...
            for (uint32_t i = 0; i < trade_id.size(); i++) {
                ...
                if (num_updated < TxnReq.max_updates) {
                    ALWAYS_ASSERT(tbl_SETTLEMENT(1/*TODO*/)->get(txn,
                                Encode(obj_key0, k_se), obj_v, std::string::npos, 96 /*access_id*/));
                    SETTLEMENT::value v_se_temp;
                    const SETTLEMENT::value *v_se = Decode(obj_v, v_se_temp);
                    ...
                    tbl_SETTLEMENT(1/*TODO*/)->put(txn, Encode(str(), k_se),
                            Encode(obj_v, v_se_new), 96 /*access_id*/);
                }else{
                    ALWAYS_ASSERT(tbl_SETTLEMENT(1/*TODO*/)->get(txn,
                                Encode(obj_key0, k_se), obj_v, std::string::npos, 96 /*access_id*/));
                    SETTLEMENT::value v_se_temp;
        ...
        // try 
        {
            ...
            for (uint32_t i = 0; i < trade_id.size(); i++) {
                ...
                if (t_is_cash_list[i]) {
                    ...
                    ALWAYS_ASSERT(tbl_CASH_TRANSACTION(1/*TODO*/)->get(txn,
                                Encode(obj_key0, k_ct), obj_v, std::string::npos, 97 /*access_id*/));
                    CASH_TRANSACTION::value v_ct_temp;
                    const CASH_TRANSACTION::value *v_ct = Decode(obj_v, v_ct_temp);
                    ALWAYS_ASSERT(v_ct);
                    //ct_name.assign(v_ct->CT_NAME.data(), v_ct->CT_NAME.size());
...
piece_retry_98:
        ...
        // try 
        {
            ...
            tbl_TRADE_T_S_SYMB(1/*TODO*/)->scan(txn, Encode(obj_key0, k_ts_0),
                    &Encode(obj_key1, k_ts_1), c, s_arena.get(), 98 /*access_id*/);
            ...
            for(size_t i = 0; i < c.size(); i++){
...

                {
                    ...
                    ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn,
                                Encode(obj_key0, k_t), obj_v, std::string::npos, 99 /*access_id*/));
                    TRADE::value v_t_temp;
                    const TRADE::value *v_t = Decode(obj_v, v_t_temp);
                    //t_ca_id = v_t->T_CA_ID;
...
piece_retry_100:
        // Raw Piece: P2 [TRADE_TYPE]
        {
            for (uint32_t i = 0; i < trade_id.size(); i++) {
                ...
                const TRADE_TYPE::key k_tt(t_tt_id_list[i].c_str());
                ALWAYS_ASSERT(tbl_TRADE_TYPE(1/*TODO*/)->get(txn,
                            Encode(obj_key0, k_tt), obj_v, std::string::npos, 100 /*access_id*/));
                TRADE_TYPE::value v_tt_temp;
                const TRADE_TYPE::value *v_tt = Decode(obj_v, v_tt_temp);
                ALWAYS_ASSERT(v_tt);
...
piece_retry_101:
        // Raw Piece: P3 [SECURITY]
        {
            ...
            ALWAYS_ASSERT(tbl_SECURITY(1/*TODO*/)->get(txn,
                        Encode(obj_key0, k_s), obj_v, std::string::npos, 101 /*access_id*/));
            SECURITY::value v_s_temp;
...
piece_retry_103:
        ...
        // catch (piece_abort_exception &ex) {
            ...
            // goto trade_update_settlement_f3;
        // }
        expose_ret = db->expose_uncommitted(txn, 103 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
            switch (expose_ret.second) {
                TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
                default: ALWAYS_ASSERT(false);
...
piece_retry_104:
        ...
        // try 
        {
            ...
            for (uint32_t i = 0; i < trade_id.size(); i++) {
                ...
                if (t_is_cash_list[i]) {
                    ...
                    if(num_updated < TxnReq.max_updates){
                        ALWAYS_ASSERT(tbl_CASH_TRANSACTION(1/*TODO*/)->get(txn,
                                    Encode(obj_key0, k_ct), obj_v, std::string::npos, 104 /*access_id*/));
                        CASH_TRANSACTION::value v_ct_temp;
                        const CASH_TRANSACTION::value *v_ct = Decode(obj_v, v_ct_temp);
                        ...
                        tbl_CASH_TRANSACTION(1/*TODO*/)->put(txn,
                                Encode(str(), k_ct), Encode(obj_v, v_ct_new), 104 /*access_id*/);
                    } else {
                        ALWAYS_ASSERT(tbl_CASH_TRANSACTION(1/*TODO*/)->get(txn,
                                    Encode(obj_key0, k_ct), obj_v, std::string::npos, 104 /*access_id*/));
                        CASH_TRANSACTION::value v_ct_temp;
        ...
        // catch (piece_abort_exception &ex) {
            ...
            // goto trade_update_cash_transaction_f3;
        // }
        expose_ret = db->expose_uncommitted(txn, 104 + ACCESSES /*access_id*/);
        if (!expose_ret.first) {
            switch (expose_ret.second) {
                TXN_TRADE_UPDATE_LIST(TXN_TRADE_UPDATE_PIECE_RETRY)
                default: ALWAYS_ASSERT(false);
...
tpce_do_test(abstract_db *db, int argc, char **argv)
{
  ...
  while (1) {
    ...
    int option_index = 0;
    int c = getopt_long(argc, argv, "i:m:w:c:s:t:", long_options, &option_index);
    if (c == -1)
      break;
    switch (c) {
    case 0:
