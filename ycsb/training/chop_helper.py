...
# dist_opts_str = "r,r,w,w,w,r,w,r,w,r"
dist_opts_str = "r,r,r,w,r,r,r,w,r,r"
dist_opts = dist_opts_str.split(',')
access = []
for i in range(len(dist_opts)):
    if dist_opts[i] == 'r':
        access.append({"operation": "read", "table": "part{%d}" % i})
    else:
        access.append({"operation": "read", "table": "part{%d}" % i})
        access.append({"operation": "write", "table": "part{%d}" % i})
...
TXN_ACCESS_NUM = [N_ACCESS]
TXN_ACCESS_START = [0, N_ACCESS]
TXN_CRITICAL_ACCESS_NUM = [N_ACCESS]
eps = 1e-5

# There is no point for us to learn wait for rendezvous points.
LEARN_WAIT = [0 for _ in range(N_ACCESS)]
TXN_ACCESS_ENDS = [N_ACCESS, 0]
