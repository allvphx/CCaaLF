...
public:
  ...
  //last commit piece id
  // volatile tid_t sub_version; 
...
//  LockG lock_graph;
  ...
  access_entry* access_head;
  access_entry* access_tail;
  volatile uint8_t tail_lock;
  mcslock *m_lock;

  uint16_t last_txn_seq;
...
#endif
  {
   //fprintf(stderr, "[dbtuple::dbtuple 1] core[%d] alloc new tuple %lx lock [%d] \n", coreid::core_id(), this, acquire_lock);
#ifdef USE_MCS_LOCK
    m_lock = new mcslock();
#endif
...
#endif
  {
   //fprintf(stderr, "[dbtuple::dbtuple 2] core[%d] alloc new tuple %lx\n", coreid::core_id(), this);
...
#endif
  {
   //fprintf(stderr, "[dbtuple::dbtuple 3] core[%d] alloc new tuple %lx\n", coreid::core_id(), this);
...
public:
  ...
  inline access_entry*
  get_latest_dependent_entries()
  {
    access_entry* dep = access_tail;
    if (dep == nullptr || dep->is_write()) {
      return dep;
    }

    while (dep->prev != nullptr && !dep->prev->is_write())
      dep = dep->prev;
  ...
  inline access_entry*
  get_dependent_entry(bool write)
  {

#ifdef TRACK_LAST_ENTRY
    return access_tail
    ...
    access_entry* dep = access_tail;
    while(dep != nullptr && !dep->is_write())
      dep = dep->prev;
  ...
  inline bool
  check_no_write_entry(uint64_t tid)
  {
    access_entry* dep = access_tail;
    while(dep != nullptr) {
      if (dep->is_write() && dep->get_tid() != tid) return false;
      dep = dep->prev;
    }
    return true;
  }
  ...
  inline bool
  is_first_active_entry(access_entry* e)
  {
    return access_head == e;
  }
  ...
  inline bool
  is_head(access_entry* e) const
  {
    return e == access_head;
  }

  inline void 
  update_tail(access_entry* e)
  {
    assert(access_tail);
    lock_tail();
    access_tail = e;
    assert((access_tail == nullptr && access_head == nullptr) ||
           (access_tail && access_head));
    unlock_tail();
  }

  inline void 
  append_entry(access_entry* e)
  {

    if(nullptr != access_tail) {
      assert(access_head != nullptr);
      e->prev = access_tail;
      access_tail->next = e;
      update_tail(e);
    } else {
      assert(access_head == NULL && access_tail == NULL);
      access_head = access_tail = e;
    }
  ...
  inline void 
  unlink_entry(access_entry *e)
  {

    assert(e->is_linked());

    if (access_tail == e && access_head == e)
    {
      //This is the only element in the access history
      
      access_head = nullptr;
      update_tail(nullptr);

    } else if (access_tail == e) {

      assert(e->prev != nullptr);

      update_tail(e->prev);
      access_tail->next = nullptr;

    } else if (access_head == e) {

      assert(e->next != nullptr);

      access_head = e->next;
      access_head->prev = nullptr;

    } else {
      
      assert(e->next != nullptr && e->prev != nullptr);
  ...
  inline version_t 
  chamcc_lock(bool lock_mode, mcslock::qnode_t *node = nullptr, bool write_intent = false)
  {
    // mode == true means spin mode
    // mode == false means mcs mode
    if (!lock_mode) {
      ALWAYS_ASSERT(node);
      return mcs_lock(node, write_intent);
    } else {
      return lock(write_intent);
    }
  }

  inline void 
  chamcc_unlock(bool lock_mode, mcslock::qnode_t *node = nullptr)
  {
    // mode == true means spin mode
    // mode == false means mcs mode
    if (!lock_mode) {
      ALWAYS_ASSERT(node);
      return mcs_unlock(node);
    } else {
      return unlock();
    }
  }
  ...
  inline version_t 
  mcs_lock(mcslock::qnode_t *node, bool write_intent = false)
  {

    //fprintf(stderr, "ACK[%d]: < %lx ,%lx>\n", coreid::core_id(), (uint64_t)this, (uint64_t)node);    
    // since spinlock is holding for all waiting mcslocks, there should not be "write intent" on spnilock
    // Instead, "write intent" should be told by mcslock
    if (!m_lock->acquire(node)) lock(false /*no write intent*/);
    
    //fprintf(stderr, "core[%d] acquire %lx lock %lx\n", coreid::core_id(), (uint64_t)this, (uint64_t)node);
    INVARIANT(!is_locked());
    ...
    // HDR_LOCKED_MASK must be already held
    const version_t lockmask = write_intent ? (HDR_WRITE_INTENT_MASK) : (0x0);
    hdr |= lockmask;
  ...
  inline void
  mcs_unlock(mcslock::qnode_t *node) 
  {
    // HDR_LOCKED_MASK must be cleared by spinlock, but we can clear HDR_MODIFYING_MASK and HDR_WRITE_INTENT_MASK
    hdr &= ~(HDR_MODIFYING_MASK | HDR_WRITE_INTENT_MASK);

    //fprintf(stderr, "Rel[%d]: < %lx ,%lx>\n", coreid::core_id(), (uint64_t)this, (uint64_t)node);    

    if (!m_lock->release(node)) unlock();
  }

  inline version_t
  lock(bool write_intent = false)
  {
...
//   while(1);
   ...
   //fprintf(stderr, "core[%d] aquire [%lx] lock\n", coreid::core_id(), this);
  ...
  unlock()
  {
   ...
   //fprintf(stderr, "core[%d] release [%lx] lock\n", coreid::core_id(), this);
    CheckMagic();
    version_t v = hdr;
    bool newv = false;
  ...
  inline bool
  last_is_deleting() const
  {
    if(access_tail != NULL) {
      if(access_tail->is_write())
        return access_tail->is_delete();
      else
        return (access_tail->get_value() == nullptr);
    }
    else {
      return is_deleting();
    }
  ...
  inline void set_last_writer_seq(int seq)
  {
    last_txn_seq = seq;
  }

  inline uint16_t get_last_writer_seq() const
  {
    return last_txn_seq;
  }
private:
...
public:
  ...
  raw_stable_read(Reader &reader, StringAllocator &sa, bool occ = true,
                  tid_t* tid = nullptr);

  template <typename Reader, typename StringAllocator>
  inline ALWAYS_INLINE bool
  ic3_stable_read(Reader &reader, tid_t& start_p, StringAllocator &sa, access_entry* e);

  //Called when a lock is hold
  template <typename Reader, typename StringAllocator>
  inline ALWAYS_INLINE bool
  ic3_read(Reader &reader, StringAllocator &sa, access_entry* e);

  //Called when a lock is hold
  template <typename Reader, typename StringAllocator>
  inline ALWAYS_INLINE bool
  profile_ic3_read(Reader &reader, StringAllocator &sa, access_entry* e, ic3_profile* prof);

  inline void
  lock_tail()
  {
    while(tail_lock  ||
          !__sync_bool_compare_and_swap(&tail_lock, 0, 1)){
      nop_pause();
    }
  }
  ...
  typedef void (*tuple_commutative_act)(std::string*, uint64_t);

  bool 
  no_append_entry()
  {
    return access_head == NULL;
  }
                  ...
                  const void *v, tuple_writer_t writer)
  {
    ...
    //FIXME: Basic assumption of ic3 -- all tuples of the same table 
    //ALWAYS_ASSERT(new_sz <= alloc_size);    
    ...
    if (likely(txn->can_overwrite_record_tid(version, t) && old_sz)) {
      ...
      dbtuple * const rep =
        alloc_spill(t, get_value_start(), old_sz, new_sz,
                    this, true, needs_old_value, access_head, access_tail, m_lock);
    ...
    const bool needs_old_value =
      writer(TUPLE_WRITER_NEEDS_OLD_VALUE, nullptr, nullptr, 0);
    dbtuple * const rep =
      alloc_spill(t, sub_version, get_value_start(), old_sz, new_sz,
                  this, true, needs_old_value, access_head, access_tail, m_lock);
    if (v)
      writer(TUPLE_WRITER_DO_WRITE, v, rep->get_value_start(), size);
    INVARIANT(rep->is_latest());
  ...
  alloc_spill(tid_t version, const_record_type value, size_type oldsz,
              ...
              bool copy_old_value, access_entry* head, access_entry* tail, mcslock* lock)
  {
    ...
    INVARIANT(p);
#ifdef USE_MCS_LOCK
    ALWAYS_ASSERT(lock != nullptr);
#endif
    return new (p) dbtuple(
        version, value, oldsz, newsz,
        alloc_sz - sizeof(dbtuple), next, set_latest, copy_old_value, head, tail, lock);
  }
