...
protected:
  ...
  struct txn_search_range_callback : public concurrent_btree::low_level_search_range_callback {
    ...
    virtual bool invoke(const typename concurrent_btree::string_type &k, typename concurrent_btree::value_type v,
                        const typename concurrent_btree::node_opaque_t *n, uint64_t version, uint32_t acc_id = 0);
  ...
  do_search(Transaction<Traits> &t,
            ...
            ValueReader &value_reader,
            bool readonly = false, uint32_t acc_id = 0);
  ...
  void do_tree_put(Transaction<Traits> &t,
                   ...
                   bool expect_new, uint32_t acc_id);
    ...
    bool readonly, uint32_t acc_id)
{
  ...
  if (found) {
    ...
    return t.do_tuple_read(tuple, value_reader, readonly, acc_id);
  } else {
    ...
    ic3_profile* prof)
{
  ...
  typename concurrent_btree::value_type underlying_v{};
  concurrent_btree::versioned_node_t search_info;
  const bool found = this->underlying_btree.profile_search(varkey(*key_str), underlying_v, &search_info, prof);
  if (found) {
    const dbtuple * const tuple = reinterpret_cast<const dbtuple *>(underlying_v);
    bool res = t.profile_do_tuple_read(tuple, value_reader, readonly, prof);
...
{
  if (acc_id) {
    t.get_state()->tx_n_op = acc_id-1;
    t.get_state()->refresh_state();
  } else {
    t.get_state()->cached_policy = nullptr;
  }
//  if (policy_abort_now(t.get_state()->state)) {
//    const transaction_base::abort_reason r = transaction_base::ABORT_REASON_LEARNED_ABORT;
//    t.abort_impl(r);
...
retry:
  ...
  assert(px);
  if (!insert) {
    // for insert, we take the state taken during read.
      if (t.get_state()->need_lock()) {
        t.release_read_lock(px);

        if(t.find_write_lock_set(px) == t.write_lock_set.end()) {
            if(!px->start_write(t.get_tid(), t.get_state(), t.not_sorted()))
            {
                const transaction_base::abort_reason r = transaction_base::ABORT_REASON_LOCK_CONFLICT;
                t.abort_impl(r);
                throw transaction_abort_exception(r);
            }
            t.write_lock_set.emplace_back(px, true);
        }
    ...
    const typename concurrent_btree::node_opaque_t *n, uint64_t version, uint32_t acc_id = 0)
{
  ...
  if (t->do_tuple_read(tuple, *value_reader, true, acc_id))
