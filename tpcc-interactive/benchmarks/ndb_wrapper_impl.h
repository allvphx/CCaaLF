...
struct hint_kv_get_put_traits {
  ...
  static const size_t write_set_expected_size = 50;
...
struct hint_micro_traits {
  static const size_t read_set_expected_size = 100;
  static const size_t write_set_expected_size = 100;
...
struct hint_micro_loader_traits {
  ...
  static const bool hard_expected_sizes = true;
  static const bool read_own_writes = false;
  static const size_t dep_queue_expected_size = 1024;
  typedef str_arena StringAllocator;
};
...
ndb_ordered_index<Transaction>::get(
    ...
    std::string &value, size_t max_bytes_read, uint32_t acc_id)
{
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) + std::string(":total:")));
...
#define MY_OP_X(a, b) \
  case a: \
    { \
      auto t = cast< b >()(p); \
      if (!btr.search(*t, key, value, acc_id, max_bytes_read)) \
        return false; \
      return true; \
    }
    switch (p->hint) {
...
template <template <typename> class Transaction>
bool
ndb_ordered_index<Transaction>::get_for_read(
    void *txn,
    const std::string &key,
    std::string &value, size_t max_bytes_read, uint32_t acc_id)
{
...
#define MY_OP_X(a, b) \
  case a: \
    { \
      auto t = cast< b >()(p); \
      if (!btr.search(*t, key, value, acc_id, max_bytes_read, true)) \
        return false; \
      return true; \
    }
    switch (p->hint) {
...
template <template <typename> class Transaction>
void
ndb_ordered_index<Transaction>::set_table_order(
    uint16_t table_order)
{
  btr.set_table_order(table_order);
}
...
ndb_ordered_index<Transaction>::put(
    void *txn,
    const std::string &key,
    const std::string &value, uint32_t acc_id)
{
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) + std::string(":total:")));
...
ndb_ordered_index<Transaction>::put(
    void *txn,
    std::string &&key,
    std::string &&value, uint32_t acc_id)
{
  ndbtxn * const p = reinterpret_cast<ndbtxn *>(txn);
  try {
#define MY_OP_X(a, b) \
...
template <template <typename> class Transaction>
void
ndb_ordered_index<Transaction>::remove(void *txn, const std::string &key, uint32_t acc_id)
{
  PERF_DECL(static std::string probe1_name(std::string(__PRETTY_FUNCTION__) + std::string(":total:")));
...
template <template <typename> class Transaction>
void
ndb_ordered_index<Transaction>::remove(void *txn, std::string &&key, uint32_t acc_id)
{
  ndbtxn * const p = reinterpret_cast<ndbtxn *>(txn);
  try {
#define MY_OP_X(a, b) \
