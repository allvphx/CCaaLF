...
#define D_SIZE (string::npos)
#define N_NEW_ORDER_ACCESS 8
#define N_DELIVERY_ACCESS 4
#define N_PAYMENT_ACCESS 4
...
uint16_t WAREHOUSE = 0;
uint16_t DISTRICT = 1;
uint16_t ITEM = 2;
uint16_t STOCK = 3;
uint16_t NEW_ORDER = 4;
uint16_t OORDER = 5;
uint16_t OORDER_INDEX = 6;
uint16_t ORDER_LINE = 7;
uint16_t CUSTOMER = 8;
uint16_t CUSTOMER_INDEX = 9;
uint16_t HISTORY = 10;
uint16_t OFFSET = 11;
...
//XXX: fake range lock for district table
static spinlock dis_range_lock[64][10];

static int32_t dist_last_id[64][10];

static inline void
acquire_dis_range_lock(int wid, int dis)
{
  ALWAYS_ASSERT(wid > 0 && wid <= 64);
  ALWAYS_ASSERT(dis > 0 && dis <= 10);

  dis_range_lock[wid - 1][dis - 1].lock();
}

static inline void
release_dis_range_lock(int wid, int dis)
{
  ALWAYS_ASSERT(wid > 0 && wid <= 64);
  ALWAYS_ASSERT(dis > 0 && dis <= 10);

  dis_range_lock[wid - 1][dis - 1].unlock();
}
  ...
  load()
  {
    ...
    try {
      ...
      for (uint i = 1; i <= NumWarehouses(); i++) {
        tbl_warehouse(i)->set_table_order((WAREHOUSE << OFFSET) + i);
      ...
      for (uint i = 1; i <= NumWarehouses(); i++) {
        const warehouse::key k(i);
        string warehouse_v;
        ALWAYS_ASSERT(tbl_warehouse(i)->get_for_read(txn, Encode(k), warehouse_v));
...
protected:
  ...
  load()
  {
    ...
    uint64_t total_sz = 0;
    try {
      tbl_item(1)->set_table_order(ITEM << OFFSET);
...
protected:
  ...
  load()
  {
    ...
    for (uint w = w_start; w <= w_end; w++) {
      tbl_stock(w)->set_table_order((STOCK << OFFSET) + w);
  ...
  load()
  {
    ...
    try {
      ...
      for (uint w = 1; w <= NumWarehouses(); w++) {
        tbl_district(w)->set_table_order((DISTRICT << OFFSET) + w);
        if (pin_cpus)
          PinToWarehouseId(w);
...
protected:
  ...
  load()
  {
    ...
    for (uint w = w_start; w <= w_end; w++) {
      tbl_customer(w)->set_table_order((CUSTOMER << OFFSET) + w);
      tbl_customer_name_idx(w)->set_table_order((CUSTOMER_INDEX << OFFSET) + w);
      tbl_history(w)->set_table_order((HISTORY << OFFSET) + w);
      if (pin_cpus)
        PinToWarehouseId(w);
...
protected:
  ...
  load()
  {
    ...
    for (uint w = w_start; w <= w_end; w++) {
      tbl_new_order(w)->set_table_order((NEW_ORDER << OFFSET) + w);
      tbl_oorder(w)->set_table_order((OORDER << OFFSET) + w);
      tbl_oorder_c_id_idx(w)->set_table_order((OORDER_INDEX << OFFSET) + w);
      tbl_order_line(w)->set_table_order((ORDER_LINE << OFFSET) + w);
      if (pin_cpus)
        PinToWarehouseId(w);
      for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
        set<uint> c_ids_s;
...
tpcc_worker::txn_new_order()
{
  ...
  std::sort(itemIDs, itemIDs+numItems);
  std::sort(supplierWarehouseIDs, supplierWarehouseIDs+numItems);
  ...
  try {
    ssize_t ret = 0;
    std::uint32_t access_id = 0;

    // access 1: read warehouse table (access 0 for no specified case's CC policy)
    access_id ++;
    const warehouse::key k_w(warehouse_id);
    ALWAYS_ASSERT(tbl_warehouse(warehouse_id)->get_for_read(txn, Encode(obj_key0, k_w), obj_v, D_SIZE, access_id));
    warehouse::value v_w_temp;
    const warehouse::value *v_w = Decode(obj_v, v_w_temp);
    checker::SanityCheckWarehouse(&k_w, v_w);

    // access 2: read modify write district table
    access_id ++;
    const district::key k_d(warehouse_id, districtID);
    ALWAYS_ASSERT(tbl_district(warehouse_id)->get(txn, Encode(obj_key0, k_d), obj_v, D_SIZE, access_id));
    ...
    if (!g_new_order_fast_id_gen) {
      ...
      v_d_new.d_next_o_id++;
      tbl_district(warehouse_id)->put(txn, Encode(str(), k_d), Encode(str(), v_d_new), access_id);
    }
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      ALWAYS_ASSERT(tbl_item(1)->get_for_read(txn, Encode(obj_key0, k_i), obj_v, D_SIZE, access_id));
      item::value v_i_temp;
    ...
    //  access 3: read modify write stocks
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      ALWAYS_ASSERT(tbl_stock(ol_supply_w_id)->get(txn, Encode(obj_key0, k_s), obj_v, D_SIZE, access_id));
      ...
      tbl_stock(ol_supply_w_id)->put(txn, Encode(str(), k_s), Encode(str(), v_s_new), access_id);
    }

    // access 4: insert new order
    access_id ++;
    const new_order::key k_no(warehouse_id, districtID, my_next_o_id);
    const new_order::value v_no;
    const size_t new_order_sz = Size(v_no);
    tbl_new_order(warehouse_id)->insert(txn, Encode(str(), k_no), Encode(str(), v_no), access_id);
    ret += new_order_sz;


    // access 5: insert order
    access_id ++;
    ...
    tbl_oorder(warehouse_id)->insert(txn, Encode(str(), k_oo), Encode(str(), v_oo), access_id);
    ret += oorder_sz;

    // access 6: insert order index.
    access_id ++;
    const oorder_c_id_idx::key k_oo_idx(warehouse_id, districtID, customerID, k_no.no_o_id);
    const oorder_c_id_idx::value v_oo_idx(0);

    tbl_oorder_c_id_idx(warehouse_id)->insert(txn, Encode(str(), k_oo_idx), Encode(str(), v_oo_idx), access_id);

    // access 7: insert order lines.
    access_id ++;
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      const uint ol_supply_w_id = supplierWarehouseIDs[ol_number - 1];
      const uint ol_i_id = itemIDs[ol_number - 1];
      ...
      tbl_order_line(warehouse_id)->insert(txn, Encode(str(), k_ol), Encode(str(), v_ol), access_id);
      ret += order_line_sz;
    }

    // access 8: read customer
    access_id ++;
    const customer::key k_c(warehouse_id, districtID, customerID);
    ALWAYS_ASSERT(tbl_customer(warehouse_id)->get_for_read(txn, Encode(obj_key0, k_c), obj_v, D_SIZE, access_id));
    customer::value v_c_temp;
    const customer::value *v_c = Decode(obj_v, v_c_temp);
    checker::SanityCheckCustomer(&k_c, v_c);

    assert(access_id == N_NEW_ORDER_ACCESS);
...
tpcc_worker::txn_delivery()
{
  ...
  try {
    ...
    // access xx: scan the new order table.
    // Instead of learning a policy for scan, we adopt a explicit lock policy.
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      acquire_dis_range_lock(warehouse_id, d);
    }

    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {

      const new_order::key k_no_0(warehouse_id, d, dist_last_id[warehouse_id - 1][d - 1]);
    ...
    // access 9: remove the read new order records.
    access_id ++;
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      tbl_new_order(warehouse_id)->remove(txn, Encode(str(), new_order_keys[d - 1]), access_id);
    }

    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      if(last_no_o_id[d - 1] != -1)
      {
        dist_last_id[warehouse_id - 1][ d - 1] = last_no_o_id[d - 1] + 1;
      }
    }

    // access 10: read modify write the order table.
    access_id ++;
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      if(last_no_o_id[d - 1] == -1)
      {
      ...
      if (unlikely(!tbl_oorder(warehouse_id)->get(txn, Encode(obj_key0, k_oo), obj_v, D_SIZE, access_id))) {
      ...
      tbl_oorder(warehouse_id)->put(txn, Encode(str(), k_oo), Encode(str(), v_oo_new), access_id);
      c_id[d - 1] = v_oo->o_c_id;
    }

    // access 11: update order line tables for delivered orders.
    access_id ++;
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      if(last_no_o_id[d - 1] == -1)
      {
      ...
      for (size_t i = 0; i < c.size(); i++) {
        ...
        tbl_order_line(warehouse_id)->put(txn, *c.values[i].first, Encode(str(), v_ol_new), access_id);
      }
      ol_total[d - 1] = sum;
    }


    // access 12: update customer tables for balance.
    access_id ++;
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      if(last_no_o_id[d - 1] == -1)
      {
      ...
      ALWAYS_ASSERT(tbl_customer(warehouse_id)->get(txn, Encode(obj_key0, k_c), obj_v, D_SIZE, access_id));

      customer::value v_c_temp;
      const customer::value *v_c = Decode(obj_v, v_c_temp);
      customer::value v_c_new(*v_c);
      v_c_new.c_balance += ol_total[d - 1];
      tbl_customer(warehouse_id)->put(txn, Encode(str(), k_c), Encode(str(), v_c_new), access_id);
    }

    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      release_dis_range_lock(warehouse_id, d);
    }

    assert(access_id == N_DELIVERY_ACCESS);
...
tpcc_worker::txn_payment()
{
  ...
  try {
    ssize_t ret = 0;
    std::uint32_t access_id = 1; // no workload information --> start from 1.

    // access 13: read modify write warehouse table, update YTD
    access_id ++;
    const warehouse::key k_w(warehouse_id);
    ALWAYS_ASSERT(tbl_warehouse(warehouse_id)->get(txn, Encode(obj_key0, k_w), obj_v, D_SIZE, access_id));
    warehouse::value v_w_temp;
    const warehouse::value *v_w = Decode(obj_v, v_w_temp);
    checker::SanityCheckWarehouse(&k_w, v_w);
    ...
    tbl_warehouse(warehouse_id)->put(txn, Encode(str(), k_w), Encode(str(), v_w_new), access_id);

    // access 14: read modify write district table, update total payment amount.
    access_id ++;
    const district::key k_d(warehouse_id, districtID);
    ALWAYS_ASSERT(tbl_district(warehouse_id)->get(txn, Encode(obj_key0, k_d), obj_v, D_SIZE, access_id));
    district::value v_d_temp;
    ...
    // access 15: read modify write user (by name or by id), update user balance.
    access_id ++;
    customer::key k_c;
    customer::value v_c;
    if (RandomNumber(r, 1, 100) <= 60) {
      ...
      ALWAYS_ASSERT(tbl_customer(customerWarehouseID)->get(txn, Encode(obj_key0, k_c), obj_v, D_SIZE, access_id));
      Decode(obj_v, v_c);
    ...
    tbl_customer(customerWarehouseID)->put(txn, Encode(str(), k_c), Encode(str(), v_c_new), access_id);

    // access 16: insert history data.
    access_id ++;
    ...
    tbl_history(warehouse_id)->insert(txn, Encode(str(), k_h), Encode(str(), v_h), access_id);
    ret += history_sz;

    assert(access_id == N_PAYMENT_ACCESS);
