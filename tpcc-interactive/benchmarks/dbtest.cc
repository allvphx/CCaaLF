...
main(int argc, char **argv)
{
  ...
  string encoder = "./encoder/default_tpcc_encoder.txt";
  ...
  while (1) {
    ...
    int option_index = 0;
    int c = getopt_long(argc, argv, "b:s:t:d:B:f:r:n:o:m:l:a:x:p:e", long_options, &option_index);
    if (c == -1)
      break;
  ...
  else if (bench_type == "tpcc_2pl")
    test_fn = tpcc_2pl_do_test;
  ...
  else if (bench_type == "microbadcount")
    test_fn = micro_badcount_do_test;
//  else if (bench_type == "seats")
//    test_fn = seats_do_test;
  else if (bench_type == "locking") 
    test_fn = micro_locking_do_test;
  else if (bench_type == "pieces")
    test_fn = micro_pieces_do_test;
  else
    ALWAYS_ASSERT(false);
  ...
  if (!policy_str.empty()) load_policy(policy_str);
