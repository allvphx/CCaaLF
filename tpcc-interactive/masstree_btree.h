...
public:
  ...
  * of setting up RCU */
  ...
  inline bool profile_search(const key_type &k, value_type &v,
                     versioned_node_t *search_info = nullptr, ic3_profile* prof = nullptr) const;
  ...
  public:
    ...
    virtual bool invoke(const string_type &k, value_type v,
                        const node_opaque_t *n, uint64_t version, uint32_t acc_id = 0) = 0;
  };
  ...
  public:
    ...
    invoke(const string_type &k, value_type v,
           const node_opaque_t *n, uint64_t version, uint32_t acc_id = 0)
    {
      return invoke(k, v, acc_id);
    }

    virtual bool invoke(const string_type &k, value_type v, uint32_t acc_id = 0) = 0;
  };
...
template <typename P>
inline bool mbtree<P>::profile_search(const key_type &k, value_type &v,
                              versioned_node_t *search_info, ic3_profile* prof) const
{
  uint64_t beg = rdtsc();
  rcu_region guard;
  prof->data[ic3_profile::level::LEVEL6] += rdtsc() - beg;

  beg = rdtsc();
  threadinfo ti;
  Masstree::unlocked_tcursor<P> lp(table_, k.data(), k.length());

  prof->data[ic3_profile::level::LEVEL7] += rdtsc() - beg;
  beg = rdtsc();

  bool found = lp.profile_find_unlocked(ti, prof);

  prof->data[ic3_profile::level::LEVEL8] += rdtsc() - beg;
  ...
  if (search_info)
    *search_info = versioned_node_t(lp.node(), lp.full_version_value());
  prof->data[ic3_profile::level::LEVEL9] += rdtsc() - beg;
...
public:
  ...
  bool
  invoke(const string_type &k, value_type v,
         const node_opaque_t *n, uint64_t version, uint32_t acc_id = 0) OVERRIDE
  {
