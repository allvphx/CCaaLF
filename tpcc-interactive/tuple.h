...
public:
  ...
  inline bool start_read(uint64_t txn_id, xact* xact, bool not_sorted = true)
  {
    return rwlock.lockR(txn_id, xact, not_sorted);
  }

  inline void end_read(uint64_t txn_id)
  {
    // fprintf(stderr, "end read  %lx %lx\n", (uint64_t)this, (uint64_t)n);
    rwlock.unlockR(txn_id);
  }

  inline bool start_write(uint64_t txn_id, xact* xact, bool not_sorted = true)
  {
      return rwlock.lockW(txn_id, xact, not_sorted);
  }

  inline void end_write(uint64_t txn_id)
  {
    // fprintf(stderr, "end write  %lx %lx\n", (uint64_t)this, (uint64_t)n);
    rwlock.unlockW(txn_id);
  }

  inline bool is_not_protected(uint64_t txn_id) const
  {
    return rwlock.lockEmpty(txn_id);
  }

  inline bool is_not_modified(uint64_t txn_id) const
  {
    return rwlock.lockNotModified(txn_id);
  }
...
public:
  ...
  read(
      tid_t t, tid_t &start_t,
      Reader &reader, StringAllocator &sa) const
  {

    INVARIANT(is_locked());

    if (unlikely(version == MAX_TID || is_deleting())) {
      start_t = MIN_TID;
      return READ_EMPTY;
    }
    ...
    ALWAYS_ASSERT(size && reader(get_value_start(), size, sa));
