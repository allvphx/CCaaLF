...
public:
  ...
  inline void put(
      ...
      FieldsMask fm = FieldsMask(), uint32_t acc_id = 0);

  template <typename Traits>
  inline void insert(
      Transaction<Traits> &t, const key_type &k, const value_type &v, uint32_t acc_id = 0);

  template <typename Traits>
  inline void remove(
      Transaction<Traits> &t, const key_type &k, uint32_t acc_id = 0);
...
typed_txn_btree<Transaction, Schema>::put(
    Transaction<Traits> &t, const key_type &k, const value_type &v, FieldsMask fm, uint32_t acc_id)
{
  ...
  this->do_tree_put(t, stablize(t, k), stablize(t, v), tw, false, acc_id);
}
...
typed_txn_btree<Transaction, Schema>::insert(
    Transaction<Traits> &t, const key_type &k, const value_type &v, uint32_t acc_id)
{
  static_assert(IsSupportable<Traits>(), "xx");
  const dbtuple::tuple_writer_t tw =
    &typed_txn_btree_<Schema>::template tuple_writer<AllFieldsMask>;
  this->do_tree_put(t, stablize(t, k), stablize(t, v), tw, true, acc_id);
}
...
typed_txn_btree<Transaction, Schema>::remove(
    Transaction<Traits> &t, const key_type &k, uint32_t acc_id)
{
  static_assert(IsSupportable<Traits>(), "xx");
  const dbtuple::tuple_writer_t tw =
    &typed_txn_btree_<Schema>::template tuple_writer<0>;
  this->do_tree_put(t, stablize(t, k), nullptr, tw, false, acc_id);
}
