...
public:
  ...
  {
    return search(t, to_string_type(k), v, acc_id, max_bytes_read, read_only);
  }
         ...
         bool read_only = false)
  {
    ...
    t.compare_lock(this->get_order(), k);
    return this->do_search(t, k, r, read_only, acc_id);
  }

  inline void
  set_table_order(uint16_t order)
  {
         ...
         ic3_profile* prof = nullptr)
  {
    ...
    single_value_reader_type r(&v, max_bytes_read);
    bool res = this->profile_do_search(t, k, r, false, prof);
  ...
  template <typename Traits>
  inline void
  put(Transaction<Traits> &t, const key_type &k, const value_type &v, uint32_t acc_id = 0)
  {
    INVARIANT(!v.empty());
    t.compare_lock(this->get_order(), k);
    this->do_tree_put(
  ...
  template <typename Traits>
  inline void
  put(Transaction<Traits> &t, const varkey &k, const value_type &v, uint32_t acc_id = 0)
  {
    INVARIANT(!v.empty());
    this->do_tree_put(
        t, stablize(t, k), stablize(t, v),
  ...
  template <typename Traits>
  inline void
  insert(Transaction<Traits> &t, const key_type &k, const value_type &v, uint32_t acc_id = 0)
  {
    INVARIANT(!v.empty());
    t.compare_lock(this->get_order(), k);
    this->do_tree_put(
  ...
  template <typename Traits>
  inline void
  insert(Transaction<Traits> &t, const key_type &k, const uint8_t *v, size_type sz, uint32_t acc_id = 0)
  {
    INVARIANT(v);
    INVARIANT(sz);
    t.compare_lock(this->get_order(), k);
  ...
  template <typename Traits>
  inline void
  insert(Transaction<Traits> &t, const varkey &k, const uint8_t *v, size_type sz, uint32_t acc_id = 0)
  {
    INVARIANT(v);
    INVARIANT(sz);
    this->do_tree_put(
  ...
  template <typename Traits>
  inline void
  remove(Transaction<Traits> &t, const key_type &k, uint32_t acc_id = 0)
  {
  ...
  template <typename Traits>
  inline void
  remove(Transaction<Traits> &t, const varkey &k, uint32_t acc_id = 0)
  {
    this->do_tree_put(t, stablize(t, k), nullptr, txn_btree_::tuple_writer, false, acc_id);
  }
