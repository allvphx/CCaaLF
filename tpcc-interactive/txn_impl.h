...
transaction<Protocol, Traits>::clear()
{
  ...
  write_set.clear();
  read_set.clear();
  early_validate_read_set.clear();
  write_lock_set.clear();
  read_lock_set.clear();
  lock_table = 0;
  lock_key = "";
  sorted = true;
...
{
#if PROFILE_LOCK
  (global_listener.abort_distribution[reason] ++);
#endif
   if (!write_lock_set.empty()) {
    auto it = write_lock_set.begin();
    auto it_end = write_lock_set.end();
    for (; it != it_end; ++it) {
      //wouldn't release write lock during execution
      dbtuple* tuple = const_cast<dbtuple *>(it->get_tuple());
      tuple->end_write(get_tid());
    }
  }

  if (!read_lock_set.empty()) {
    typename lock_set_map::iterator it = read_lock_set.begin();
    typename lock_set_map::iterator it_end = read_lock_set.end();
    for (; it != it_end; ++it) if(it->get_locked()) {
        dbtuple* tuple = const_cast<dbtuple *>(it->get_tuple());
        tuple->end_read(get_tid());
    }
  }
  clear();
}
    ...
    bool did_group_insert)
{
  if (did_group_insert) {
    ...
    INVARIANT(last.entry->get_tuple()->is_locked());
    INVARIANT(last.get_tuple()->is_locked());
  } else {
    ...
    assert(dbtuple::IsLatest(v) == tuple->is_latest());
    last.mark_locked();
...
template <template <typename> class Protocol, typename Traits>
bool
transaction<Protocol, Traits>::early_validation() {
    // do early validation, w/o lock.
    // check the nodes we actually read are still the latest version
    // records are only checked once.
    if (!early_validate_read_set.empty()) {
//       get_state()->conflict_mask &= ~F_CURRENT_IS_RISKY;
       typename read_set_map::iterator it     = early_validate_read_set.begin();
       typename read_set_map::iterator it_end = early_validate_read_set.end();
       for (; it != it_end; ++it) {
        if (likely(it->get_tuple()->is_latest_version(it->get_tid())))
            continue;
        abort_trap((reason = ABORT_REASON_EARLY_VALIDATION_FAIL));
        abort_impl(reason);
        return false;
       }
       early_validate_read_set.clear();
...
transaction<Protocol, Traits>::commit(bool doThrow)
{
  ...
  if (!write_set.empty()) {
    ...
    for (size_t pos = 0; it != it_end; ++it, ++pos) {
      ...
      assert(!it->is_insert() || tuple->is_locked());
      write_dbtuples.emplace_back(tuple, &(*it), it->is_insert(), pos);
    }
  }
  ...
  if (!is_snapshot()) {
    ...
    if (!write_dbtuples.empty()) {
      ...
      for (; it != it_end; last_px = &(*it), ++it) {
        if (likely(last_px && last_px->get_tuple() != it->get_tuple())) {
          // on boundary
          if (unlikely(!handle_last_tuple_in_group(*last_px, inserted_last_run))) {
              // TODO: the last_tuple (has MAX_ID version, not locked, not inserted).
              abort_trap((reason = ABORT_REASON_WRITE_NODE_INTERFERENCE));
            goto do_abort;
          }
          assert(last_px->get_tuple()->is_locked());
...
//     do read validation
    ...
    if (!write_dbtuples.empty()) {
      ...
      for (auto it = write_dbtuples.begin(); it != write_dbtuples.end(); it++) {
        auto tuple = it->get_tuple();
        if (it->is_locked())
          tuple->unlock();
        else
    ...
    if (!write_lock_set.empty()) {
      typename lock_set_map::iterator it = write_lock_set.begin();
      typename lock_set_map::iterator it_end = write_lock_set.end();
      for (; it != it_end; ++it) {
        dbtuple* tuple = const_cast<dbtuple *>(it->get_tuple());
        tuple->end_write(get_tid());
      }
...
do_abort:
  ...
  for (auto it = write_dbtuples.begin();
    ...
    dbtuple::tuple_writer_t writer)
{
  ...
  if (get_state()->need_lock())
  {
      auto it_r = find_write_set(tuple);
      if (it_r == write_set.end())
      {
          if (!tuple->start_write(get_tid(), get_state(), not_sorted()))
          {
              const transaction_base::abort_reason r = transaction_base::ABORT_REASON_LOCK_CONFLICT;
              abort_impl(r);
              throw transaction_abort_exception(r);
          }
          write_lock_set.emplace_back(tuple, true);
      }
  }
...
transaction<Protocol, Traits>::do_tuple_read(
    const dbtuple *tuple, ValueReader &value_reader, bool readonly, uint32_t acc_id)
{
  INVARIANT(tuple);
  ...
  if (is_snapshot_txn) assert(acc_id == 0);
  if (acc_id) {
    get_state()->tx_n_op = acc_id-1;
    get_state()->refresh_state();
  } else {
    get_state()->cached_policy = nullptr;
  }
    ...
    if(is_snapshot_txn) {
      ...
      if (!get_state()->need_lock()) {
        ...
        if (stat != dbtuple::READ_EMPTY) {
            early_validate_read_set.emplace_back(tuple, start_t);
        }
      } else {
        dbtuple* t_ptr = const_cast<dbtuple*>(tuple);
        if(readonly) {
          auto it_r = find_read_set(t_ptr);
          if (it_r == read_set.end())
          {
            if (!t_ptr->start_read(get_tid(), get_state(), not_sorted()))
            {
              const transaction_base::abort_reason r = transaction_base::ABORT_REASON_LOCK_CONFLICT;
              abort_impl(r);
              throw transaction_abort_exception(r);
            }
            read_lock_set.emplace_back(tuple, true);
            stat = tuple->stable_read(snapshot_tid, start_t, value_reader, this->string_allocator(), is_snapshot_txn);
          } else {
            stat = tuple->stable_read(snapshot_tid, start_t, value_reader, this->string_allocator(), is_snapshot_txn);
          }
          ...
          if(stat == dbtuple::READ_EMPTY) {
            t_ptr->end_read(get_tid());
          } else {
            read_set.emplace_back(tuple, start_t);
          }

        } else {
          auto it_r = find_write_set(t_ptr);
          if (it_r == write_set.end())
          {
            if (!t_ptr->start_write(get_tid(), get_state(), not_sorted()))
            {
              const transaction_base::abort_reason r = transaction_base::ABORT_REASON_LOCK_CONFLICT;
              abort_impl(r);
              throw transaction_abort_exception(r);
            ...
            write_lock_set.emplace_back(tuple, true);
            stat = tuple->read(snapshot_tid, start_t, value_reader, this->string_allocator());
          } else {
            stat = tuple->read(snapshot_tid, start_t, value_reader, this->string_allocator());
          }

          if(stat == dbtuple::READ_EMPTY) {
            if (get_state()->need_lock()) t_ptr->end_write(get_tid());
          } else {
            read_set.emplace_back(tuple, start_t);
          }
        }
...
template <typename ValueReader>
bool
transaction<Protocol, Traits>::profile_do_tuple_read(
    const dbtuple *tuple, ValueReader &value_reader, bool readonly, ic3_profile* prof)
{
  return false;
}
