...
#define ABORT_REASONS(x) \
    ...
    x(ABORT_REASON_LOCK_CONFLICT)\
    x(ABORT_REASON_LEARNED_ABORT)\
    x(ABORT_REASON_VALIDATION_LOCK_FAIL)\
    x(ABORT_REASON_EARLY_VALIDATION_FAIL)
...
protected:
  ...
  struct lock_record_t {
    constexpr lock_record_t() : tuple(), locked() {}
    constexpr lock_record_t(const dbtuple *tuple, bool locked)
      : tuple(tuple), locked(locked) {}
...
protected:
  ...
  typedef small_vector<
    lock_record_t,
    traits_type::write_set_expected_size> lock_set_map_small;
  ...
  typedef static_vector<
    lock_record_t,
    traits_type::write_set_expected_size + traits_type::read_set_expected_size> lock_set_map_static;
  ...
  typedef
    typename std::conditional<
      traits_type::hard_expected_sizes,
      lock_set_map_static, lock_set_map_small>::type lock_set_map;
  typedef
    typename std::conditional<
      traits_type::hard_expected_sizes,
  ...
  typedef std::vector<write_record_t> lock_set_map;
...
protected:
  ...
  bool
  do_tuple_read(const dbtuple *tuple, ValueReader &value_reader, bool readonly = false, uint32_t acc_id = 0);

  template <typename ValueReader>
  bool
  do_tuple_read_nonblock(const dbtuple *tuple, ValueReader &value_reader);
  

  template <typename ValueReader>
  bool
  profile_do_tuple_read(const dbtuple *tuple, ValueReader &value_reader, bool readonly = false, ic3_profile * prof = nullptr);
...
public:
  ...
  xact* get_state() const {
      return feature;
  }

  bool not_sorted() const {
    return !sorted;
  }

  // uint64_t get_lock_address() const {
  //   return lock_address;
  // }

  // void update_lock_address(uint64_t addr) {
  //   lock_address = addr;
  // }
  void compare_lock(uint16_t table, std::string key) {
    sorted = true;
    if (table < lock_table || (table == lock_table && key.compare(lock_key) < 0))
    {
      sorted = false;
    }
    if (sorted)
...
protected:
  ...
  inline void release_read_lock(const dbtuple* tuple)
  {
    typename lock_set_map::iterator it_r = find_read_lock_set(tuple);
    if (it_r != read_lock_set.end())
    {
      it_r->unlock();
  ...
  typename lock_set_map::iterator
  find_read_lock_set(const dbtuple *tuple)
  {
    // linear scan- returns the *first* entry found
    // (a tuple can exist in the read_set more than once)
    ...
    for (; it != it_end; ++it)
      if (it->get_tuple() == tuple && it->get_locked())
        break;
    return it;
  }

  typename lock_set_map::iterator
  find_write_lock_set(const dbtuple *tuple)
  {
    // linear scan- returns the *first* entry found
    // (a tuple can exist in the read_set more than once)
  ...
  read_set_map early_validate_read_set;
  absent_set_map absent_set;

  lock_set_map read_lock_set;
  lock_set_map write_lock_set;
  uint16_t lock_table;
  std::string lock_key;
  bool sorted = false;
